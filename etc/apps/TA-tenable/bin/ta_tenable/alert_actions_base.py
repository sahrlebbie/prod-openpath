...
class ModularAlertBase(ModularAction):
    ...
    def build_http_connection(self, config, timeout=120,
        ...
        import httplib2_wrapper # puts TA level httplib2 lib in sys.path based on python version
    ...
    def get_events(self):
        try:
            ...
            textfile = codecs.getreader("utf-8")(self.result_handle)
            return (self.pre_handle(num, result) for num, result in enumerate(csv.DictReader(textfile)))
        except IOError:
            msg = "Error: {}."
    ...
    def prepare_meta_for_cam(self):
        with gzip.open(self.results_file, 'rb') as rf:
            textfile = codecs.getreader("utf-8")(rf)
