...
class ModinputEventWriter(object):
    ...
    def __init__(self, process_safe=False):
        if process_safe:
            ...
            self._event_queue = self._mgr.Queue(2)
        else:
...
class HecEventWriter(object):
    ...
    def _prepare_events(self, events):
        ...
        for evt in events:
            ...
            if size + len(evt) + 1 >= self._max_event_size:
                if batched:
                    all_events.append("\n".join(batched))
                del batched[:]
                size = 0
            ...
            for _ in xrange(retry):
                try:
                    ...
                    if "Broken pipe" in e.message:
                        # urllib3 behavior
                        self._do_handle_large_error(event)
                    time.sleep(2)
    ...
    def create_event(index, host, source, sourcetype,
                     time, unbroken, done, events, fields=None):
    ...
    def _do_handle_large_error(self, event):
        hcc = hecc.HECConfig(
            self._config["server_uri"], self._config["session_key"])
        hcc.reload_http_input()
        limits = hcc.get_limits()
        if int(limits["max_content_length"]) >= len(event):
            return

        new_limit = {
            "max_content_length": len(event) + 1
        }

        if new_limit["max_content_length"] < self.max_length:
            hcc.set_limits(new_limit)
            hcc.reload_http_input()
            # Close all exiting pooled TCP connections to workaround
            # HEC issue since HEC server need refresh all connections
            if hasattr(self._http, "clear") and callable(self._http.clear):
                self._http.clear()
            return True
        else:
