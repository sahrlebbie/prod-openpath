...
class CloudWatchSpace(object):
    ...
    def __init__(self, region, account, iam_role):
        self._region = region
    ...
    def run(self, app, config, io):
        ...
        client = AWSEnvironment(credentials, region)
        executor = MetricQueryExecutor(region, credentials, io)
...
class CloudWatchSpaces(object):
    ...
    def __call__(self, region, account, iam_role):
        key = region, account, iam_role
...
class DataInput(object):
    ...
    def _create_spaces(self, spaces):
        ...
        return [spaces(region, account, iam_role) for region in regions]
