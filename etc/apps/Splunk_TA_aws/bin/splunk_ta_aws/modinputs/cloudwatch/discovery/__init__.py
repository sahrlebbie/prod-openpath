...
class CloudWatchMetricsCache(Cache):
    ...
    def load(self, credentials, region, session):
        logger.debug('Start listing metrics.', namespace=self._namespace)
        client = credentials.client('cloudwatch', region, session)
...
class EC2ReservationsCache(Cache):
    def load(self, credentials, region, session):
        ...
        client = credentials.client('ec2', region, session)
...
class AutoScalingGroupsCache(Cache):
    def load(self, credentials, region, session):
        ...
        client = credentials.client('autoscaling', region, session)
...
class SQSQueuesCache(Cache):
    def load(self, credentials, region, session):
        ...
        client = credentials.client('sqs', region, session)
...
class SNSTopicsCache(Cache):
    def load(self, credentials, region, session):
        ...
        client = credentials.client('sns', region, session)
...
class LambdaFunctionsCache(Cache):
    def load(self, credentials, region, session):
        ...
        client = credentials.client('lambda', region, session)
...
class ELBInstancesCache(Cache):
    def load(self, credentials, region, session):
        ...
        client = credentials.client('elb', region, session)
...
class ELBV2TargetGroupsCache(Cache):
    def load(self, credentials, region, session):
        ...
        client = credentials.client('elbv2', region, session)
...
class S3BucketsCache(Cache):
    def load(self, credentials, region, session):
        ...
        client = credentials.client('s3', region, session)
...
class AWSEnvironment(object):
    ...
    def _acquire(self, cls, *args):
        ...
        if key not in self._caches:
            cache = cls(*args)
            cache.load(self._credentials, self._region, self._session)
