...
class Metric(object):
    ...
    def _make_state_key(cls, namespace, name, dimensions):
        ...
        sha.update(name)
        sha.update(json.dumps(dimensions, sort_keys=True))
    ...
    def restore(cls, quadruples):
        return [Metric(*args) for args in izip(*quadruples)]
...
class MetricQueryResult(object):
    ...
    def __iter__(self):
        return izip(self._metrics, self._points)
...
class MetricQueryExecutor(object):
    ...
    def __init__(self, region, credentials, io):
    ...
    def _create_client(self, session):
        return self._credentials.client('cloudwatch', self._region, session)
