...
require([
        ...
        'splunkjs/mvc/simplexml/ready!' // ensures configuration.xml is loaded before the call to this function
    ],
    function ($, _, modal, mvc, Messages, SearchManager, SimpleView) {
        ...
        var TestConnectionView = SimpleView.extend({
            ...
            _completeDialog: function () {
                ...
                var header = $('#testConnectionDialog_header').text('Connection test for ' + $('#domain').val());
        ...
        var configurationEditor = {
            ...
            save: function () {
                ...
                $('#domain,#alternatedomain,#basedn,#server').each(function (index, target) {
                    self._validateRequiredInputElement(target);
                });

                $('#domain,#alternatedomain').each(function (index, target) {
                ...
                var input = $('#domain,#alternatedomain,#basedn,#server,#port,#ssl,#binddn,#password').parent();
            ...
            test: function () {
                ...
                self.save().done(function () {
                    ...
                    var domainName = $('#domain').val().trim();
                    ...
                    $('#testConnectionDialog').modal('show');
            ...
            _alert: function (type, title, message) {
                ...
                body.html('<i class="icon-alert"/>' + text);
                return $('#alertDialog').modal('show');
            },
            ...
            _confirm: function (type, title, message, commands) {
                ...
                $('#confirmDialog_body')
                    ...
                    .html('<i class="icon-alert"/>' + message);
                ...
                var dialog = $('#confirmDialog').modal({show: true, backdrop: 'static'});
            ...
            _bindEventHandlers: function () {
                ...
                $('#domains').on('change', self.select);
                $('#add').click(self.add);
                $('#remove').click(self.remove);
                $('#save').click(self.save);
                $('#test').click(self.test);

                //// ...for Inputs

                $('#domain,#alternatedomain,#basedn,#server,#port,#ssl,#binddn,#password').on('change', function () {
                    self._isDirty(true);
                });

                $('#domain,#alternatedomain,#basedn,#server').on('change', function (event) {
                    self._validateRequiredInputElement(event.target);
                });

                $('#domain,#alternatedomain').on('change', function (event) {
            ...
            _validateDomainName: function (target) {
                ...
                if (option === nameMap[domains.children('[selected=selected]').val()]) {
                    if ($('#domain').val() != $('#alternatedomain').val()) {
                        self._clearInputError(target);
                        return true;
                    }
