...
require(['jquery'], function(){
        ...
        $.when(window_data_inventory_eventtypes_established).then(function(){
            ...
            let intro_steps = {
                "step-init": {
                    ...
                    "start": function(id, callback){
                        ...
                        generateSearchManager("| tstats prestats=t count where index=* NOT index=_* earliest=-14d@d latest=@d by index sourcetype | eval source=\"N/A\" | tstats append=t prestats=t count where sourcetype=*winevent* index=* NOT index=_* earliest=-14d@d latest=@d by index source| eval sourcetype=coalesce(sourcetype, \"wineventlog\") | stats count by index source sourcetype", "GLOBAL_SOURCETYPE_SOURCE_INDEX", {"autostart": true, "override_auto_finalize": 100}, function(id, data) {
                    ...
                    "status": function(){
                        ...
                        $("#step-init_status").text((success + failure) + ' ' + _('complete').t() + ' / ' + searching + ' ' + _('currently searching').t() + ' / ' + pending + ' ' + _('queued').t() )
                ...
                "step-cim": {
                    ...
                    "handleResults": function(id, callback){
                        ...
                        for(let i = 0; i < data.length; i++){
                            let searchString = "index=\"" + data[i].index + "\" "
                ...
                "step-sourcetype": {
                    ...
                    "start": function(id, callback){
                        ...
                        if(object["term_fields"] && object["term_fields"] != ""){
                            ...
                            search = "| loadjob " + intro_elements.GLOBAL_SOURCETYPE_SOURCE_INDEX.sid + " | search " + object['basesearch'] + " | stats sum(count) as count by " + object['splitby']
                        }
                        ...
                        generateSearchManager(search, "step-sourcetype-" + object['productId'], parameters, function(id, data) {
                                ...
                                id=id.replace("step-sourcetype-", "")
                                // console.log("STEP-SOURCETYPE Got a started search", id)
                                if(splunkjs.mvc.Components.getInstance(id) && splunkjs.mvc.Components.getInstance(id).attributes && splunkjs.mvc.Components.getInstance(id).attributes.data && splunkjs.mvc.Components.getInstance(id).attributes.data.sid){
                                    intro_elements[id]["search_details"]["sid"] = splunkjs.mvc.Components.getInstance(id).attributes.data.sid;
                                }
                                intro_steps["step-sourcetype"].updateDisplay(id)
                ...
                "step-eventsize": {
                    ...
                    "start": function(id, callback){
                        ...
                        function(id, data) {
                            ...
                            id=id.replace("step-eventsize-", "")
                            // console.log("Got a started search", id)
                            if(splunkjs.mvc.Components.getInstance(id) && splunkjs.mvc.Components.getInstance(id).attributes && splunkjs.mvc.Components.getInstance(id).attributes.data && splunkjs.mvc.Components.getInstance(id).attributes.data.sid){
                                intro_elements[id]["search_details"]["sid"] = splunkjs.mvc.Components.getInstance(id).attributes.data.sid;
                            }
                            intro_steps["step-eventsize"].updateDisplay(id)
                ...
                "step-volume": {
                    ...
                    "start": function(id, callback){
                        ...
                        generateSearchManager("| tstats count dc(host) as numhosts where " + object.termsearch + " earliest=-30d@d latest=@d by _time span=1d | stats avg(count) as avg_count avg(numhosts) as avg_numhosts ", "step-volume-" + id, 
                    ...
                    "handleResults": function(id, callback){
                        ...
                        if(object["desired_sampling_ratio"] && object["desired_sampling_ratio"] != ""){
            ...
            function requestKVStoreUpdate(collection_name, updateTime){
                ...
                if(kvstore_update_queue[collection_name]['status'] == 0){
                    ...
                    $.ajax({
                        ...
                        error: function(){
                            triggerError("Got an error while trying to update the kvstore. Your changes may not be saved.")
            ...
            function huntForNewAnalyzingProducts(){
                for(let i = 0; i < window.data_inventory_products.length; i++){
                    if(window.data_inventory_products[i].status == "analyzing" && intro_elements[ window.data_inventory_products[i].productId ] && intro_elements[ window.data_inventory_products[i].productId ].stage != "step-eventsize"){
            ...
            function genericUpdateDisplay(id, stage){
                ...
                if(status == "searching"){
                    ...
                    if(intro_elements[id]["search_details"] && intro_elements[id]["search_details"]["sid"]){
                        row.find(".result").html($("<a class=\"external drilldown-link\" target=\"_blank\">").attr("href", "search?sid=" + splunkjs.mvc.Components.getInstance(id).attributes.data.sid))
                    }
                }else if(status == "failure" || status == "error"){
                    row.find(".cancelSearch").html("")
                    row.find(".running-status").html( _("Search Complete").t() )
                    row.find(".result").html( _("Did not find any data").t() )
                    ...
                    row.find(".cancelSearch").html("")
                    row.find(".running-status").html( _("Search Complete").t() )
                    row.find(".result").html( _("Found data").t() )
            ...
            function genericPoll(id, stage){
                if(intro_elements[id].status == "searching"){
                        ...
                        setTimeout(function(){
                            if(intro_elements[id].status == "searching"){
                                if(! intro_elements[id].search_details){ 
                                    ...
                                    // }
                                    intro_elements[id].status = "failure"
                                    intro_elements["jsonStatus"][intro_elements[id].stage]['status'] = "failure"
                        ...
                        setTimeout(function(){
                            ...
                            results.on("data", function(properties) {
                                ...
                                if (data && data.length && data.length >= 1) {
                                    intro_elements[id]["search_result"] = JSON.stringify(data);
                                    intro_elements[id].status = "complete"
                                    intro_elements["jsonStatus"][intro_elements[id].stage]['status'] = "complete"
                                    intro_steps[stage].handleResults(id)
                    ...
                    //     intro_steps[stage].updateDisplay(id)
                    //     revokeSM(id)
                    }else if(intro_elements[id].search_details.search_started - Math.round(Date.now() / 1000) > 60){
                        // If the search has been running for over a minute
            ...
            function reportTelemetry(id, counter){
                ...
                if(element.vendorName){
                    if(window.valid_ootb_products.vendorName.indexOf(element.vendorName) >= 0){
            ...
            function updateKVStore(id, updateOnly, forceUpdate){
                ...
                if(/^DS\d\d\d[A-Z]/.test(id) || /^VendorSpecific/.test(id)){
                    ...
                    let kvstore_fields = ["_key", "created_time", "eventtypeId", "status", "basesearch", "search_result", "coverage_level"]
            ...
            $.when(initComplete, window_data_inventory_products_established).then(function(){
                ...
                for( let i = 0; i < data_inventory_products.length; i++){
                    ...
                    if(data_inventory_products[i]['termsearch'] && data_inventory_products[i]['termsearch'] != "" && data_inventory_products[i]['termsearch'].indexOf("sourcetype=") >= 0){
                        let matches = Array.from(  data_inventory_products[i]['termsearch'].matchAll(/sourcetype="*([^\s"]+)/) );
                        ...
                        let matches = Array.from(  data_inventory_products[i]['termsearch'].matchAll(/source="*([^\s"]+)/) );
                        for(let g = 0; g < matches.length; g++){
                            global_sourcetype_to_vendor.push({
                                "match": true,
                                "productName": data_inventory_products[i]['productName'],
