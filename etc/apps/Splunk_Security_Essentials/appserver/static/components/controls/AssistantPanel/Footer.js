        ...
        function AssistantPanelFooter(panel) {
            ...
            if (footerButtons !== false) {
                if (footerButtons.openInSearchButton !== false) {
                    panelElements.openInSearchButton = $('<button>').addClass('btn btn-default').text(_('Open in Search').t() );
                }

                if (footerButtons.showSPLButton !== false) {
                    panelElements.showSPLButton = $('<button>').addClass('btn btn-default mlts-show-spl').text(_('Line-by-Line SPL Documentation').t());
                }

                if (footerButtons.scheduleAlertButton === true) {
                    panelElements.scheduleAlertButton = $('<button>').addClass('btn btn-default schedule-alert-button').text(_('Save Scheduled Search').t());
                }

                if (footerButtons.scheduleHighCardinalityAlertButton === true) {
                    panelElements.scheduleHighCardinalityAlertButton = $('<button>').addClass('btn btn-default').text(_('Schedule High Cardinality Alert').t());
                }
