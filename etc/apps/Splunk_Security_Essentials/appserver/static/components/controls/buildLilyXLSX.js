...
let sheets = {
    ...
    "sheet6": {
        "columns": [{"ref": getRefForString("Security Domain"), "width": 15}, {"ref": getRefForString("Title"), "width": 53}, {"ref": getRefForString("Description"), "width": 64}, {"ref": getRefForString("Use Case(s)"), "width": 24}, {"ref": getRefForString("Journey Stage"), "width": 17}, {"ref": getRefForString("Category"), "width": 18}, {"ref": getRefForString("Data Source Category"), "width": 35}, {"ref": getRefForString("MITRE ATT&CK Tactic"), "width": 22}, {"ref": getRefForString("MITRE ATT&CK Technique"), "width": 42}, {"ref": getRefForString("Kill Chain Phase"), "width": 22},{"ref": getRefForString("CIS Controls"), "width": 30},{"ref": getRefForString("NIST Category"), "width": 30}, {"ref": getRefForString("Data Models"), "width": 30}, {"ref": getRefForString("Providing Technologies"), "width": 50}],
        "rows": [],
        "order": 6,
        "name": "ES Content Update (Detections)",
        "displayapp": "Enterprise Security Content Update",
        "keysToHarvest": ["domain", "name", "description", "usecase", "journey", "category", "data_source_categories_display", "mitre_tactic_display", "mitre_technique_display", "killchain", "escu_cis", "escu_nist", "escu_data_models", "escu_providing_technologies"]
    },
    "sheet7": {
    ...
    "sheet9": {
        ...
        "rows": [],
        "order": 9,
        "name": "Splunk Phantom (Highlights)",
        // "descriptionRow": "Note: This sheet does not list all Phantom Community Playbooks, just some of the most requested, most popular playbooks. To see all that Phantom can deliver, reach out to your Splunk team or check out our github at: https://github.com/phantomcyber/playbooks.",
...
// Init Intro Sheet Content
        ...
        for(let storyId in ShowcaseInfo.escu_stories){
            ...
            for(let x = 0; x < story.detections.length; x++){
                ...
                if(ShowcaseInfo.summaries[summaryName]){
                    ...
                    let mvFields = ["category", "killchain", "mitre_technique_display", "mitre_tactic_display", "escu_cis", "escu_nist", "escu_data_models", "escu_providing_technologies"];
    ...
    let supportsMV = ["domain", "usecase", "category", "data_source_categories_display", "mitre_tactic_display", "mitre_technique_display", "killchain", "escu_nist", "escu_cis", "escu_data_models", "escu_providing_technologies", "contributing_anomalies", "detection_methods", "contributes_to"]
        ...
        if(AppNameToSheet[ShowcaseInfo['summaries'][summaryName]['displayapp']]){
            ...
            for(let i = 0; i < sheets[sheetId].keysToHarvest.length; i++){
                ...
                if(supportsMV.indexOf(sheets[sheetId].keysToHarvest[i]) >= 0){
                    ...
                    row.push(getRefForString( ShowcaseInfo['summaries'][summaryName][ sheets[sheetId].keysToHarvest[i] ] ))
                }
        ...
        for(let sheet in sheets){
            ...
            if(sheets[sheet]['sheetXML']){
                ...
                if(sheets[sheet]['descriptionRow'] && sheets[sheet]['descriptionRow'] != ""){
                    ...
                    descriptionRow += "</row>"
                    // console.log("Adding the following row for phantom", descriptionRow, "with content", sharedStrings[sheets[sheet]['descriptionRow']])
                    rowCounter++;
                }
