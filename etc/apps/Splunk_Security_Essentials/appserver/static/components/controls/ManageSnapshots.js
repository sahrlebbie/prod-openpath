function manageContentModal() {
    ...
    let body = $("<div>").append($('<p>' + _('Here you can control your Splunk Security Essentials bookmarks and custom content. You can snapshot your configurations, restore them, export them, or more. Snapshots are stored in a lookup called sse_bookmark_backup.csv -- if you would like to back them up off system, you can create a snapshot and then back up this file.').t() + '</p>'),
        $('<p>' + _('Importantly, the snapshots here are inclusive of both Bookmarks and Custom Content. You can check or uncheck one or the other, as desired.').t() + '</p>'),
        $('<button style="" class="btn">' + _('Clear Current List').t() + '</button>').click(function() {
            $(".modal,.modal-backdrop").remove()
            clearBookmarkList();
        }),
        $('<button style="margin-left: 8px;" class="btn">' + _('Snapshot Current List').t() + '</button>').click(function() {
            $(".modal,.modal-backdrop").remove()
            snapshotBookmarkList();
        }),
        $('<button style="margin-left: 8px;" class="btn">' + _('Import New Snapshot').t() + '</button>').click(function() {
            $(".modal,.modal-backdrop").remove()
            importBookmarkList();
        }),
        $('<button style="margin-left: 8px;" class="btn btn-primary">' + _('Manage and Restore Snapshots').t() + '</button>').click(function() {
            $(".modal,.modal-backdrop").remove()
            restoreBookmarkList();
        }))
...
function importBookmarkList() {
    ...
    require([
        ...
        function(
            ...
            let body = $('<p>' + _('On the Manage Snapshots page, you can export a snapshot dump (base64 encoded JSON). If you\'d like to import a dump, please paste it into the box below. You will then be taken to the Manage Snapshots page and can opt to restore it if you would like.').t() + '</p><div id=\"importStatus\"><textarea id=\"importSnapshotBlob\" /></div>')
                ...
                try {
                    ...
                    try {
                        ...
                        importSnapshotSearch.on('search:fail', function(properties) {
                            $("#importStatus").html('<h3>' + _('Error!').t() + '</h3><p>' + _('Unknown error appending snapshot to the lookup.').t() + '</p>')
                        })
                        ...
                        importSnapshotSearch.on('search:done', function(properties) {
                            ...
                            let myModal = new Modal('importSnapshotSuccess', {
                                title: _('Import Snapshot Success').t()
                            });
                            $(myModal.$el).on("hide", function() {
                                // Not taking any action on hide, but you can if you want to!
                            })

                            let body = $('<p>' + Splunk.util.sprintf(_('Success! Imported snapshot \"%s.\" It has now been added to your snapshot list, you may restore it if desired.').t(), output['name']) + '</p>')
                            ...
                            myModal.footer.append($('<button>').attr({
                                ...
                                type: 'button',
                                'data-dismiss': 'modal'
                            }).addClass('btn btn-primary').text( _('Restore Snapshots').t() ).on('click', function() {
                                $(".modal,.modal-backdrop").remove()
                                restoreBookmarkList()
                            }))
                            myModal.show()
...
function restoreBookmarkList() {
    ...
    require([
        ...
        function(
            ...
            let body = $('<p>' + _('Below you will find a list of all existing snapshots. Click the restore button to make them current, or the <i class=\"icon-close\"/> to remove them.').t() + '</p><div id=\"listOfSnapshots\"><h3>' + _('Processing...').t() + '</h3>')
            ...
            snapshotCollectSearch.on('search:done', function(properties) {
                ...
                if (properties.content.resultCount == 0) {
                    $("#listOfSnapshots").html('<h3>' + _('No Snapshots Found').t() + '</h3>')
                } else {
                    ...
                    results.on("data", function(properties) {
                        ...
                        let table = $('<table id="snapshotList" class="table table-striped"><thead><tr><th>' + _('Snapshot Name').t() + '</th><th>' + _('When Captured').t() + '</th><th>' + _('User').t() + '</th>' + 
                                            '<th>' + _('# Bookmarks').t() + '</th>' + '<th>' + _('# Custom Items').t() + '</th>' + 
                                            '<th>' + _('# Mappings').t() + '</th>' + '<th>' + _('Is Data Inventory Included').t() + '</th>' + 
                        ...
                        for (let i = 0; i < data.length; i++) {
                            ...
                            row.append($('<td class="snapshot-">').html($('<i class="icon-export" style="cursor: pointer;">').click(function(evt) {
                                ...
                                let body = $("<div>")
                                body.append('<p>' + _('Below is the contents of this export (a JSON output encoded as base64). You can bring this to any Splunk Security Essentials instance and paste it into the text box on the Import Snapshot window from the Manage List link on the Bookmarked Content page (just as you got to this window).').t() + '</p>')
                                body.append($('<textarea id="importSnapshotBlob" />').text(base64Output))
                                body.append($('<p> ' + _('to automatically add the text to your clipboard').t() + '</p>').prepend($('<a href="#">' + _('Click Here').t() + '</a>').click(function() {
