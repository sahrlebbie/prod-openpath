...
function checkForMITREInLogReview(deferral){
    ...
    $.ajax({
        ...
        success: function(returneddata) {
            if(! returneddata || returneddata.length == 0 || Object.keys(returneddata).length == 0){
                deferral.resolve("errored")
            }
            var mystanza = {}
            ...
            let addMITRETactic_Combined = true;
            let addMITRETechnique = true;
            let addMITRETechnique_Combined = true;
            let addMITREDescription_Combined = true;
            let addKillChain = true;
            for(let i = 0; i < newEventAttributes.length; i++){
                if(newEventAttributes[i].field == "mitre_technique"){
                    addMITRETechnique = false;
                }
                if(newEventAttributes[i].field == "mitre_technique_combined"){
                    addMITRETactic_Combined = false;
                }
                if(newEventAttributes[i].field == "mitre_technique_description"){
                    addMITREDescription_Combined = false;
                }
                if(newEventAttributes[i].field == "mitre_tactic"){
                    addMITRETactic = false;
                }
                if(newEventAttributes[i].field == "mitre_tactic_combined"){
                    addMITRETechnique_Combined = false;
                }
            ...
            if(addMITRETechnique){
                shouldUpdate = true;
                newEventAttributes.unshift({"field": "mitre_technique", "label": "MITRE ATT&CK Technique ID"}) 
            }
            if(addMITRETechnique_Combined){
                shouldUpdate = true;
                newEventAttributes.unshift({"field": "mitre_technique_combined", "label": "MITRE ATT&CK Technique"}) 
            }
            if(addMITREDescription_Combined){
                shouldUpdate = true;
                newEventAttributes.unshift({"field": "mitre_technique_description", "label": "MITRE ATT&CK Technique Description"}) 
            }
            if(addMITRETactic){
                shouldUpdate = true;
                newEventAttributes.unshift({"field": "mitre_tactic", "label": "MITRE ATT&CK Tactic ID"}) 
            }
            if(addMITRETactic_Combined){
                shouldUpdate = true;
                newEventAttributes.unshift({"field": "mitre_tactic_combined", "label": "MITRE ATT&CK Tactic"}) 
            }
            if(addKillChain){
                shouldUpdate = true;
                newEventAttributes.unshift({"field": "killchain", "label": "Kill Chain Phase"}) 
            }
...
function CheckWhatAppsArePresent(deferral){
    ...
    let apps = {
        ...
        "SA-cim_validator": "sse-setup-app-sa-cim"
    }
    let returnObj = {}
    $.ajax({
        ...
        success: function(returneddata) {
            for(let i = 0; i < returneddata.entry.length; i++){
                ...
                if(apps[returneddata.entry[i].name]){
                    ...
                    returnObj[ apps[returneddata.entry[i].name] ] = {"status": "installed", "version": returneddata.entry[i].content.version }
                ...
                if(apps[returneddata.entry[i].content.label]){
                    ...
                    returnObj[ apps[returneddata.entry[i].content.label] ] = {"status": "installed", "version": returneddata.entry[i].content.version }
                    delete apps[returneddata.entry[i].content.label]
                }
            }
            for(let key in apps){
                // console.log("No luck with", key, apps[key])
                returnObj[ apps[key] ] = {"status": "not_installed", "version": "N/A"}
...
function CheckIfDataInventoryComplete(deferral){
    ...
    require([
        ...
        'json!' + $C['SPLUNKD_PATH'] + '/servicesNS/nobody/Splunk_Security_Essentials/storage/collections/data/data_inventory_products?bust=' + Math.round(Math.random() * 50000),
        'json!' + $C['SPLUNKD_PATH'] + '/servicesNS/nobody/Splunk_Security_Essentials/storage/collections/data/data_inventory_eventtypes?bust=' + Math.round(Math.random() * 50000),
        'json!' + $C['SPLUNKD_PATH'] + '/services/pullCSV?config=sse-default-products',
    ], function(
        $,
        _,
        //data_inventory,
        ...
        for(let i = 0; i < sse_default_products.length; i++){
            if(! (sse_default_products[i]['default_sourcetype_search'] && sse_default_products[i]['vendorName'] && sse_default_products[i]['productName'])){
                continue;
            }
            let productId = ""
        ...
        for(let i = 0; i < data_inventory_products.length; i++){
            if(! (data_inventory_products[i].stage == "step-sourcetype" && (data_inventory_products[i].status == "pending" || data_inventory_products[i].status == "blocked" || data_inventory_products[i].status == "new"))){
                // console.log("Looking at a product", data_inventory_products[i].productId, data_inventory_products[i], hash_products_default)
                if(hash_products_default[data_inventory_products[i].productId]){
                    products_default_checked++;
                }else{
                    products_custom_total++;
                    if(data_inventory_products[i].stage == "needsConfirmation"){
...
function CheckIfSearchesAreMapped(deferral){
    ...
    $.ajax({
        ...
        async: true,
        success: function(savedSearchObj) {
            savedSearchesDeferral.resolve(savedSearchObj.entry)
        }
    })
    ...
    require([
        ...
        'underscore',
        //'json!' + $C['SPLUNKD_PATH'] + '/services/pullJSON?config=data_inventory&locale=' + window.localeString,
        'json!' + $C['SPLUNKD_PATH'] + '/servicesNS/nobody/Splunk_Security_Essentials/storage/collections/data/bookmark?bust=' + Math.round(Math.random() * 50000),
        'json!' + $C['SPLUNKD_PATH'] + '/servicesNS/nobody/Splunk_Security_Essentials/storage/collections/data/local_search_mappings?bust=' + Math.round(Math.random() * 50000),
        'json!' + $C['SPLUNKD_PATH'] + '/servicesNS/nobody/Splunk_Security_Essentials/storage/collections/data/custom_content?bust=' + Math.round(Math.random() * 50000)
    ], function(
        $,
        _,
        //data_inventory,
        ...
        $.when(savedSearchesDeferral).then(function(saved_searches){
            ...
            for(let i = 0; i < saved_searches.length; i++){
                if (saved_searches[i].content['disabled'] == false && saved_searches[i].content.cron_schedule.length >= 9 && ! (saved_searches[i].name.indexOf(" - Lookup Gen") >= 0 || saved_searches[i].name.indexOf(" - Threat Gen") >= 0 || saved_searches[i].name.indexOf(" - Context Gen") >= 0 ||
                (standardESApps.indexOf(saved_searches[i].acl.app) >= 0 && ! saved_searches[i].content['action.notable'] == "1") || standardIrrelevantApps.indexOf(saved_searches[i].acl.app) >= 0)) {
                    num_saved_searches++;
