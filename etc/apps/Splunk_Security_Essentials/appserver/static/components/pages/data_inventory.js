        ...
        for(let ds in data_inventory){
            ...
            for(let dsc in data_inventory[ds]['eventtypes']){
                ...
                if($(".ds_datasource#" + dsc).length > 0){
                    if(! eventtypeList[dsc] || eventtypeList[dsc] == priorities["starting"]){
                        $(".ds_datasource#" + dsc).find(".ds_status").find("div").html('<i class="icon-question" style="position: absolute; left: 6px; top: 6px; font-size: 18px" />')
                    }else if(eventtypeList[dsc] == priorities["searchRan"]){
                        $(".ds_datasource#" + dsc).find(".ds_status").find("div").html('<i class="icon-close" style="color: red; position: absolute; left: 6px; top: 6px; font-size: 18px" />')
                        
                    }else if(eventtypeList[dsc] == priorities["haveData"]){
                        $(".ds_datasource#" + dsc).find(".ds_status").find("div").html('<i class="icon-check" style="color: green; position: absolute; left: 6px; top: 6px; font-size: 18px" />')
                        
                    }else if(eventtypeList[dsc] == priorities["needsCoverage"]){
                        $(".ds_datasource#" + dsc).find(".ds_status").find("div").html('<i class="icon-pencil" style="color: yellow; position: absolute; left: 6px; top: 6px; font-size: 18px" />')
                        
                    }else if(eventtypeList[dsc] == priorities["needsReview"]){
                        $(".ds_datasource#" + dsc).find(".ds_status").find("div").html('<i class="icon-gear" style="color: darkred; position: absolute; left: 6px; top: 6px; font-size: 18px" />')
                        
                    }
                }
                // console.log("Random Status", "a:", dsc, "b:", status, "c:", eventtypeList[dsc], "d:", eventtypeCoverageLevels[dsc])
            }
            let icon = '<i class="icon-question" style="margin-left: 5px; font-size: 22px" />'
            if(status == priorities["searchRan"]){
                icon = '<i class="icon-close" style="color: red; margin-left: 5px; font-size: 22px" />'
            }else if(status == priorities["haveData"]){
                icon = '<i class="icon-check" style="color: green; margin-left: 5px; font-size: 22px" />'
            }else if(status == priorities["needsCoverage"]){
                icon = '<i class="icon-pencil" style="color: yellow; margin-left: 5px; font-size: 22px" />'
            }else if(status == priorities["needsReview"]){
                icon = '<i class="icon-gear" style="color: darkred; margin-left: 5px; font-size: 22px" />'
            }
            if(countAboveZero == count){
                ...
                if(icon.indexOf("icon-gear") >= 0 || icon.indexOf("icon-pencil") >= 0){
                    ...
                    $(".datasource_main#" + ds).find("h2").first().html( $(".datasource_main#" + ds).find("h2").first().text().replace(/ \([\d\/]*\).*/, "") + " (" + countAboveZero + "/" + count + ")  " + '<i class="icon-question" style="margin-left: 5px; font-size: 22px" />' ) 
                }
            }
            ...
            for(let i = 0; i < window.data_inventory_products.length; i++){
                if(window.data_inventory_products[i].productId == productId){
                    if( window.data_inventory_products[i].status 
                        && (dataProductStatusNames[window.data_inventory_products[i].status] 
