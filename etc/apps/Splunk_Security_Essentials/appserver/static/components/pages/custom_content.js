require(["components/controls/Modal", "underscore"], function(Modal, _){
...
require([
        ...
        Splunk.util.make_full_url("/static/app/Splunk_Security_Essentials/components/controls/ManageSnapshots.js")
    ],
    function(
        $,
        _,
        ...
        $("#bookmark_table").append("<table style=\"\" id=\"main_table\" class=\"table table-chrome\" ><thead><tr class=\"dvbanner\">" +
            ...
            "<th style=\"text-align: center\"><span data-placement=\"top\" data-toggle=\"tooltip\">Remove </span></th>" +
            "<th style=\"text-align: center\"><span data-placement=\"top\" data-toggle=\"tooltip\">Export </span></th>" +
        ...
        $.getJSON($C['SPLUNKD_PATH'] + '/services/SSEShowcaseInfo?locale=' + window.localeString, function(ShowcaseInfo) {
            ...
            for (let i = 0; i < ShowcaseList.length; i++) {
                if (!ShowcaseInfo.summaries[ShowcaseList[i]].channel || ShowcaseInfo.summaries[ShowcaseList[i]].channel != "custom") {
                    delete ShowcaseInfo.summaries[ShowcaseList[i]]
                    if (ShowcaseInfo.roles.default.summaries.indexOf(ShowcaseList[i]) >= 0) {
                        ShowcaseInfo.roles.default.summaries.splice(ShowcaseInfo.roles.default.summaries.indexOf(ShowcaseList[i]), 1);
                    }
                }
            }
...
$("#restoredDeletedContent").click(function() {
    ...
    $.ajax({
        url: $C['SPLUNKD_PATH'] + '/servicesNS/nobody/Splunk_Security_Essentials/storage/collections/data/deleted_custom_content',
        type: 'GET',
        contentType: "application/json",
        async: false,
        success: function(data) {
            ...
            if (data.length == 0) {
                ...
                let table = $('<table id="restoreList" class="table table-striped"><thead><tr><th>Source</th><th>When Removed</th><th>User who Removed</th><th>Name</th><th>Restore</th><th>Purge</th></tr></thead><tbody></tbody></table>')
                let tbody = table.find("tbody")
                    // console.log("Got my data!", data)
                for (let i = 0; i < data.length; i++) {
                    let row = $("<tr>").attr("data-showcaseid", data[i]['showcaseId']).attr("data-key", data[i]['_key']).attr("data-json", data[i]['json'])
                    ...
                    row.append($('<td class="recycling-channel">').text(data[i]['channel']))
                    row.append($('<td class="recycling-time">').text(data[i]['_time']))
                    row.append($('<td class="recycling-user">').text(data[i]['user']))
                    row.append($('<td class="recycling-name">').text(showcaseName))
                    ...
                    row.append($("<td>").append($("<button class=\"btn\">").text("Restore").click(function(evt) {
                        ...
                        let showcaseId = row.attr("data-showcaseid")
                        ...
                        $.ajax({
                            url: $C['SPLUNKD_PATH'] + '/servicesNS/nobody/Splunk_Security_Essentials/storage/collections/data/custom_content/?query={"_key": "' + showcaseId + '"}',
                            type: 'GET',
                            contentType: "application/json",
                            async: false,
                                    ...
                                    $.ajax({
                                        ...
                                        success: function(returneddata) {
                                            ...
                                            setTimeout(function() {
                                                ...
                                                $.ajax({
                                                    url: $C['SPLUNKD_PATH'] + '/services/SSEShowcaseInfo?locale=' + window.localeString + "&bust=" + Math.round(Math.random() * 20000000),
                                                    async: true,
                                                    success: function(returneddata) {

                                                        $.ajax({
                                                            url: $C['SPLUNKD_PATH'] + '/servicesNS/nobody/Splunk_Security_Essentials/storage/collections/data/deleted_custom_content/' + key,
                                                            type: 'DELETE',
                                                            async: true,
                                                            success: function(returneddata) {
                                                                row.remove()
                                                        ...
                                                        myModal.body.html($("<div>").append($("<p>This custom content has been restored.</p>")))
                                        ...
                                        error: function() {
                                            triggerError("Error restoring deleted content.")
                                        }
                                    })
                                } else {
                                    triggerError("Couldn't restore content. Existing content with the same id already exists. First remove " + ShowcaseInfo['summaries'][returneddata[0]['showcaseId']]['name'])
                                }
                            },
                            error: function(error, data, other) {
                                triggerError("Unknown error restoring");
                            }
                        });
                    ...
                    row.append($("<td>").append($("<i class=\"icon-close\">").css("cursor", "pointer").click(function(evt) {
                        ...
                        let showcaseId = row.attr("data-showcaseid")
                        let key = row.attr("data-key");
                        let name = row.find(".recycling-name").text();
                        let time = row.find(".recycling-time").text();
                        let myModal = new Modal('confirmPurge', {
                            title: 'Confirm Purge',
                            destroyOnHide: true,
                            type: 'wide'
                        });

                        myModal.body.html($("<div>").append($("<p>Are you sure you would like to purge this content?</p><table class=\"table\"><thead><tr><th>Name</th><th>Time</th></tr></thead><tbody><tr><td>" + name + "</td><td>" + time + "</td></tr></tbody></table>")))
                        myModal.footer.append($('<button>').attr({
                            type: 'button',
                            'data-dismiss': 'modal'
                        }).addClass('btn btn-primary ').text('Cancel'), $('<button>').attr({
                            type: 'button'
                        }).addClass('btn btn-primary ').text('Purge').click(function() {
                            ...
                            $.ajax({
                                ...
                                success: function(returneddata) {
                                    ...
                                    myModal.body.html($("<div>").append($("<p>Success!</p>")))
                                    myModal.footer.append($('<button>').attr({
                                        type: 'button',
                                        'data-dismiss': 'modal'
                                    }).addClass('btn btn-primary ').text('Close'))
            ...
            myModal.body.html($("<p>Below is a listing of content that can be restored</p>"))
...
function removeContent(obj) {
    ...
    let body = $("<div>").append($("<p>Are you sure you would like to remove <i>" + showcaseName + "</i>? Your deleted custom content will be persisted in the Recycling Bin and can be restored later if needed.</p>"))
    ...
    myModal.footer.append($('<button>').attr({
        ...
        type: 'button',
        'data-dismiss': 'modal'
    }).addClass('btn btn-primary ').text('Remove').on('click', function() {
        $.when(haveCurrentObj).then(function(existingObj) {
            if (typeof existingObj == "object" && existingObj['json'] && existingObj['showcaseId']) {
                let record = {
                    _time: (new Date).getTime() / 1000,
                ...
                $.ajax({
                    ...
                    success: function(returneddata) {
                        ...
                        $.ajax({
                            url: $C['SPLUNKD_PATH'] + '/servicesNS/nobody/Splunk_Security_Essentials/storage/collections/data/custom_content/' + showcaseId,
                            type: 'DELETE',
                            async: true,
                            success: function(returneddata) {
                                bustCache(); 

                                $("[data-showcaseid=" + showcaseId + "]").remove()
                                ...
                                myModal.body.html($("<div>").append($("<p>This custom content has been removed. It can be restored from the recycling bin.</p>")))
                                myModal.footer.append($('<button>').attr({
                                    type: 'button',
                                    'data-dismiss': 'modal'
                                }).addClass('btn btn-primary ').text('Close'))
                            ...
                            error: function(xhr, textStatus, error) {
                                ...
                                myModal.body.html($("<div>").append($("<p>Error removing content.</p>"), $("<pre>").text(textStatus)))
                                myModal.footer.append($('<button>').attr({
                                    type: 'button',
                                    'data-dismiss': 'modal'
                                }).addClass('btn btn-primary ').text('Close'))
                    ...
                    error: function(xhr, textStatus, error) {
                        ...
                        triggerError("Error Adding Existing Entry to Recycling Bin. Please reach out to sse@splunk.com for assistance, referencing your scenario and this error: " + xhr.responseText);
                    }
                })
            } else {
                triggerError("Error retrieving existing key to facilitate backup (deleted_custom_content collection not present or functioning?). Please reach out to sse@splunk.com for assistance.");
            }
        })
...
function noContentMessage() {
    ...
    $("#bottomTextBlock").html("<h3>No Custom Content Created</h3><p>Please click <i>Add Custom Content</i> in the upper right box, to add new content.</p>")
...
function addItem(summary) {

    require(["jquery",
            "underscore",
            Splunk.util.make_full_url("/static/app/Splunk_Security_Essentials/components/controls/ProcessSummaryUI.js")
        ],
        function($,
            _,
            ProcessSummaryUI
            ...
            let timeStr = date.toLocaleString(window.localeString, { month: 'numeric', year: 'numeric', day: 'numeric', hour: 'numeric', minute: 'numeric', second: 'numeric', hour12: false });
            let titleRow = $("<tr class=\"titleRow\" data-showcaseId=\"" + summary.id + "\"  class=\"dvbanner\">" +
                "<td class=\"tableexpand\" class=\"downarrow\" ><a href=\"#\" onclick=\"doToggle(this); return false;\"><i class=\"icon-chevron-right\" /></a></td>" +
                "<td class=\"name content-name\"><div></div><a href=\"#\" onclick=\"doToggle(this); return false;\">" + summary.name + "</a></td>" +
                "<td style=\"text-align: center\">" + linkToExample + "</td>" +
                "<td class=\"content-user\" style=\"text-align: center\">" + summary.custom_user + "</td>" +
                "<td class=\"content-time\" style=\"text-align: center\">" + timeStr + "</td>" +
                '<td style="text-align: center" class="tableedit" ><i class="icon-pencil" style="font-size: 20px;"  name="' + summary.id + '" data-showcaseid="' + summary.id + '" data-name="' + summary.name + '" onclick="editContent(this)" ></td>' +
                '<td style="text-align: center" class="tableclose" ><i class="icon-close" style="font-size: 20px;"  name="' + summary.id + '" data-status="none" data-showcaseid="' + summary.id + '" data-name="' + summary.name + '" onclick="removeContent(this)" ></td>' +
                "</tr>")
            titleRow.attr("data-json", JSON.stringify(summary).replace(/\\/g, '\\\\').replace(/"/g, '\\"').replace(/\|/g, '\\|'))
            titleRow.append($('<td  style="text-align: center" class="snapshot-">').html($('<i class="icon-export" style="cursor: pointer;">').click(function(evt) {
                let target = $(evt.target);
                let row = target.closest("tr");
                let output = {}
                ...
                output["json"] = JSON.parse([row.attr("data-json").replace(/\\\|/g, "|").replace(/\\"/g, '"')])
            ...
            let descriptionRow = $("<tr class=\"descriptionRow\" data-showcaseId=\"" + summary.id + "\"  style=\"display: none;\"><td colspan=\"10\">" + description + "</td></tr>")
            if ($(".titleRow[data-showcaseid=" + summary.id + "]").length > 0) {
                $("#bookmark_printable_table").find("div[data-showcaseid=" + summary.id + "]").remove()
                $("#bookmark_printable_table").append($("<div>").addClass("printable_section").attr("data-showcaseid", summary.id).append($("<h1>" + summary.name + "</h1><h2>Status</h2><p>" + status_display + "</p>" + "<h2>App</h2><p>" + summary.displayapp + "</p>" + $(description.replace(/display: none/g, "")).find("#contentDescription").html()), SPL, printableimage))
                $(".titleRow[data-showcaseid=" + summary.id + "]").replaceWith(titleRow)
                $(".descriptionRow[data-showcaseid=" + summary.id + "]").replaceWith(descriptionRow)
            } else {
...
function checkForOutOfDateCustomContent() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            for (let i = 0; i < data.length; i++) {
                if (data[i].showcase_name.indexOf("MIGRATED TO NEW CUSTOM CONTENT") == -1) {
                    isOldContentPresent = true;
                }
            }

            if (isOldContentPresent) {
                $(".dashboard-view-controls").prepend($("<button style=\"margin-left: 5px;\" class=\"btn\"><b>Migrate Legacy Content</b></button>").click(function() {

                    require([Splunk.util.make_full_url("/static/app/Splunk_Security_Essentials/components/controls/CustomContent.js")], function() {

                        let table = $('<table id="oldContentList" class="table table-striped"><thead><tr><th>Name</th><th>Data Source</th><th>Status</th><th>User Who Created</th><th>When Created</th><th>Restore</th><th>Delete</th></tr></thead><tbody></tbody></table>')
                        let tbody = table.find("tbody")
                        for (let i = 0; i < data.length; i++) {
                            let row = $("<tr>").attr("data-name", data[i]['showcase_name']).attr("data-key", data[i]['_key']).attr("data-journey", data[i]['journey']).attr("data-description", data[i]['description']).attr("data-datasource", data[i]['datasource']).attr("data-status", data[i]['status'])
                            let deleteDeferral = $.Deferred();

                            row.append($('<td class="oldcontent-name">').text(data[i]['showcase_name']))
                            row.append($('<td class="oldcontent-datasource">').text(data[i]['datasource']))
                            row.append($('<td class="oldcontent-status">').text(data[i]['status']))
                            row.append($('<td class="oldcontent-user">').text(data[i]['user']))
                            row.append($('<td class="oldcontent-time">').text(data[i]['_time']))
                            ...
                            row.append($("<td>").append($("<button class=\"btn\">").text("Restore").click(function(evt) {
                                ...
                                let summary = {
                                    ...
                                    "journey": row.attr("data-journey"),
                                    "description": row.attr("data-description"),
                                    "bookmark_status": row.attr("data-status")
                                }

                                customContentModal(function(showcaseId, summary) {
                                        // console.log("Successfully Added", showcaseId, summary)
                                        deleteDeferral.resolve(key, false)
                                            // There's some processing that occurs in SSEShowcaseInfo and we want to get the full detail here.
                                        $.ajax({
                                            url: $C['SPLUNKD_PATH'] + '/services/SSEShowcaseInfo?locale=' + window.localeString + "&bust=" + Math.round(Math.random() * 20000000),
                                            async: true,
                            ...
                            row.append($("<td>").append($("<i class=\"icon-close\">").css("cursor", "pointer").click(function(evt) {
                                let target = $(evt.target);
                                let row = target.closest("tr")
                                deleteDeferral.resolve(row.attr("data-key"), true)
                                    //console.log("Got a request to delete", row.attr("data-key"))

                            })))
                            tbody.append(row)

                            let myModal = new Modal('legacyContent', {
                            ...
                            $.when(deleteDeferral).then(function(key, shouldPopModal) {
                                if (key) {
                                    $.ajax({
                                        ...
                                        success: function(returneddata) {
                                            ...
                                            if (shouldPopModal) {
                                                let myModal = new Modal('contentRemoved', {
                                                    title: 'Removed',
                                                    destroyOnHide: true,
                                                    type: 'wide'
                                                });

                                                myModal.body.html("<p>Success!</p>")
                                                myModal.footer.append($('<button>').attr({
                                                    type: 'button',
                                                    'data-dismiss': 'modal'
                                                }).addClass('btn btn-primary ').text('Close'))
                                        ...
                                        error: function(xhr, textStatus, error) {
                                            triggerError("Error deleting! " + textStatus)
                                        }
                                    })
                                }
                            })
