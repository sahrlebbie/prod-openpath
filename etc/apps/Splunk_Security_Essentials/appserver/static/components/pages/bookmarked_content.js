    ...
    function(
        ...
        if (typeof localStorage[appName + "-i18n-" + window.localeString] != "undefined" && localStorage[appName + "-i18n-" + window.localeString] != "") {
            ...
            if (langObject['build'] == build) {
                ...
                if (window.location.href.indexOf("127.0.0.1") >= 0 || window.location.href.indexOf("localhost") >= 0) { // only refresh in a dev env (or at least one without latency), otherwise it's not necessary as long as the build is the same.
...
require([
    ...
    function(
        ...
        function popModalToLookForEnabledContent(alternateParagraphElement) {
            ...
            let myModal = new Modal('confirmActive', {
            ...
            if (typeof alternateParagraphElement != "undefined") {
                ...
                myModal.body.html($("<div>").append( $("<p>" + _("In order to help you prioritize new content via the Analytics Advisor dashboards, it's useful to be able to show your existing levels of coverage and areas of focus. To make this as easy as possible, this app includes a workflow for listing all of your local saved searches and then either mapping them to Splunk's out-of-the-box-content, or creating new content in Splunk Security Essentials that you can tag with all of the metadata you care about.").t() + "</p>"), $("<p>" + _("Finally, remember that you can always come back here and change any of these settings.").t() + "</p>") ) )
            }


            myModal.body.append($('<button class="btn btn-primary">' + _("Look for Enabled Content").t() + '</button>').click(function(evt) {
                $("#confirmActive").modal("hide")

                let myModal = new Modal('localContentLoading', {
                ...
                myModal.body.html($("<p>Please wait a moment while we gather data.</p>"))
                ...
                require([Splunk.util.make_full_url("/static/app/Splunk_Security_Essentials/components/controls/MapExistingSearchContent.js")], function() {
                    // Let loose the beast
                })
            }))
        ...
        $.getJSON($C['SPLUNKD_PATH'] + '/services/SSEShowcaseInfo?locale=' + window.localeString, function(ShowcaseInfo) {
            ...
            for (let i = 0; i < ShowcaseList.length; i++) {
                if (!ShowcaseInfo.summaries[ShowcaseList[i]].bookmark_status || ShowcaseInfo.summaries[ShowcaseList[i]].bookmark_status == "none") {
...
function triggerNotes(obj) {
    ...
    require([
        'components/controls/Modal'], function(Modal){
            ...
            myModal.body
                .append($("<p>").text("Insert Notes Below, and click Save to record those notes for the future."), $('<textarea data-showcaseid="' + showcaseId + '" id="bookmark_notes" style="width: 100%; height: 300px;"></textarea>').text(existingNotes));
...
function addUseCase() {
    require([
        ...
        $(myModal.$el).addClass("modal-extra-wide").on("show", function() {

        })
        let body = $("<div>")
        body.append("<p>Please enter a search term below. The top 10 matches will then show up below. You can click the <i class=\"external drilldown-link\" /> icon to open the content in a new tab to get more detail, or click anywhere else in the tile to select it.</p>")

        var timeoutId = 0;
        body.append($('<input id="searchBar" type="text" style="width: 300px" aria-label="Input" />').on('keyup', function(e) {
        ...
        function doSearch() {
            ...
            if (results.length > 10) {
                $("#searchResultCount").text("Showing 10 out of " + results.length + " results.")
            } else {
                $("#searchResultCount").text("Showing all " + results.length + " results.")
            }
            var tiles = $('<ul class="showcase-list"></ul>')

            for (var i = 0; i < results.length && i < 10; i++) {
                if (typeof ShowcaseInfo['summaries'][results[i].ref] != "undefined") {
                    let tile = $(BuildTile.build_tile(ShowcaseInfo['summaries'][results[i].ref], true))
                    ...
                    tile.click(function(evt) {
                        let target = $(evt.target);
                        let showcaseId = target.closest("li").find(".contentstile").attr("data-showcaseid");
                        ...
                        if (target.prop('tagName') != "A") {
                            ...
                            $("#searchResults").append($("<p class=\"itemSelected\" data-showcaseid=\"" + showcaseId + "\">Selected: " + ShowcaseInfo.summaries[showcaseId].name + ' </p>').append($('<a href="#" class="icon-close" style="color: gray" />').click(function() {
                                $(".itemSelected").remove()
                                $(".showcase-list").show()
                                $("#searchResultCount").show()
                                $("#addBookmarkButton").attr("disabled", "disabled")
                            })))

                            $("#searchResults").append($('<select data-field="bookmark_status" class="   " title="" >\
                                ...
                                if (newValue == "none") {
                                    ...
                                    $("#addBookmarkButton").removeAttr("disabled")
                                }
                            }))
                        }
        ...
        myModal.footer.append($('<button>').attr({
            ...
            type: 'button',
            'data-dismiss': 'modal'
        }).addClass('btn btn-primary').attr("id", "addBookmarkButton").attr("disabled", "disabled").text('Add').on('click', function() {
            ...
            ShowcaseInfo.summaries[showcaseId].bookmark_status = status;
            ShowcaseInfo.summaries[showcaseId].bookmark_status_display = BookmarkStatus[status];

            addItem(ShowcaseInfo.summaries[showcaseId])
            window.ShowcaseInfo.summaries[showcaseId] = ShowcaseInfo.summaries[showcaseId];
