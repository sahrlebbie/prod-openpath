    ...
    function($,
        ...
        var outlierResultsSearchSettings = {
            'findNewValues': {
                ...
                description: ['Here we use the stats command to calculate what the earliest and the latest time is that we have seen this combination of fields.',
                    'This line is for convenience, where we store the current timestamp so that we can use it in the next line.'
                ]
            },
            'findNewValues-Demo': {
                search: ['| stats earliest(_time) as earliest latest(_time) as latest  by $outlierValueTracked1Token$, $outlierValueTracked2Token$ ',
                ...
                description: ['Find where the most recent value is less than -1d@d from either now() or the value showing your most recent data point (depending on your particular search desires)',
                    'In order to facilitate demo data, we are using the eventstats command to calculate what the "latest" time is. In real scenarios, you can skip this step, and just use now() instead of maxlatest later.'
                ]
            },
            'findNewValues-Lookup': {
                ...
                description: ['When running over the last 24 hours, find the earliest and latest value per pair.',
                    ...
                    'Clear out any pairs that are older than 90 days (a common request, as if something has not happened in 90 days, it is basically brand new -- this also prevents our lookups from getting needlessly large.)',
            ...
            'findNewValues-Demo-Lookup': {
                ...
                description: ['Run over a short period of time (a few hours, maybe a day), to find the most recent earliest and latest values per pair.',
                    'Append the results with our historical cache (our baseline), and use stats to recalculate the true earliest and latest per pair.',
                    'Output those results to our lookup so that we are able to use them next time.',
            ...
            'findNewValuesPeer-Lookup': {
                ...
                description: ['Run over a short period of time (a few hours, maybe a day), to find the most recent earliest and latest values per pair.',
                    ...
                    'Clear out any pairs that are older than 90 days (a common request, as if something has not happened in 90 days, it is basically brand new -- this also prevents our lookups from getting needlessly large.)',
                    'Output those results to our lookup so that we are able to use them next time.',
                    'To allow our search to also work with demo data (constraint of how this page was built), we define what "now" is in a variable that we can compare against momentarily. You could simplify by replacing maxlatest with now() below. ',
                    'Enrich the primary field with the peer group. If a lookup holds that jsmith is in the same team as msmith and charris, we will now have the usernames jsmith, msmith, and charris in a multi-value field.',
                    //'Pull out the \'Secondary Field\'s that a \'Primary Field\' has viewed, and the \'Secondary Field\' their peers have viewed',
                    'Here we are comparing the # of \'Secondary Field\'s viewed today and historically by the \'Primary Field\'. multireport is a search operator that allows you to leverage the power of stats, but multiple times.',
                    'Now we join the two | stats output together into one, so that we can analyze them together',
                    'Filtering out null earliest will handle corner cases to make a clean report.'
            ...
            'findNewValuesPeer-Demo-Lookup': {
                ...
                description: ['Run over a short period of time (a few hours, maybe a day), to find the most recent earliest and latest values per pair.',
                    ...
                    'Clear out any pairs that are older than 90 days (a common request, as if something has not happened in 90 days, it is basically brand new -- this also prevents our lookups from getting needlessly large.)',
                    'Output those results to our lookup so that we are able to use them next time.',
                    'This line is only used with demo data, where we can\'t use eval\'s "now()" function since our demo data is older. We are calculating an alternative "now()"',
                    'Enrich the primary field with the peer group. If a lookup holds that jsmith is in the same team as msmith and charris, we will now have the usernames jsmith, msmith, and charris in a multi-value field.',
                    'Here we are comparing the # of \'Secondary Field\'s viewed today and historically by the \'Primary Field\'. multireport is a search operator that allows you to leverage the power of stats, but multiple times. ',
                    'Now we join the two | stats output together into one, so that we can analyze them together',
                    'Filtering out null earliest will handle corner cases to make a clean report.'
        ...
        function setupSearches() {
            ...
            (function setupOutliersVizSearch() {
                ...
                setTimeout(function() {
                    $("#showSPLMenu").append($("<button>Line-by-Line SPL Documentation</button>").click(function() { ModifiedLaunchQuerySPL('SearchModal', 'Show Search Query', vizQueryArray, getFullSearchQueryComments(), baseTimerange, vizOptions); }))
                }, 1500)
                ...
                function scheduleAlert() {
                    require(["components/data/sendTelemetry", 'json!' + $C['SPLUNKD_PATH'] + '/servicesNS/nobody/Splunk_Security_Essentials/storage/collections/data/sse_app_config'], function(Telemetry, appConfig) {
                        let record = { "status": "scheduleAlertStarted", "name": currentSampleSearch.label }
                        ...
                        (function() {
                            ...
                            outliersVizAlertModal.footer.append($('<button>').addClass('mlts-modal-cancel').attr({
                                ...
                                if (isValid) {
                                    ...
                                    window.saveSearchComplete_Deferral = $.Deferred()


                                    // console.log("About to set Params", window.actions)
                                    if (typeof window.actions.UBASeverity != "undefined" && window.actions.UBASeverity > 0) {
                                    ...
                                    if (typeof window.actions.createNotable != "undefined" && window.actions.createNotable == 1) {
                                        ...
                                        // console.log("ParamGettingSet", shouldContinue)
                                        if (shouldContinue) {
                                            // console.log("ParamBeingSet for createNotable", window.actions.createNotable);

                                            $.when(window.saveSearchComplete_Deferral).done(function() {

                                                setTimeout(function() {

                                                    // console.log("Firing on", $(".modal-body:contains(You can view your alert, or continue editing it.)") )
                                                    $(".modal-body:contains(You can view your alert, or continue editing it.)").css("min-height", "450px")
                                                    $(".modal-body:contains(You can view your alert, or continue editing it.)").html('<p>You can view your alert, or continue editing it.</p><br /><h4>Enabling ES Correlation Search</h4><div id="ESCorrelationProcessing"></div>')
                                                    require([Splunk.util.make_full_url("/static/app/Splunk_Security_Essentials/vendor/spin/spin.min.js")], function(Spinner) {
                                                        new Spinner({ scale: 0.5, lines: 8, top: "10px", position: "relative" }).spin(document.getElementById('ESCorrelationProcessing'));
                                                    })

                                                }, 500)

                                                var searchName = dvtestalertmodal.model.alert.entry.content.attributes.name;
                                                setTimeout(function() {
                                                    ...
                                                    var mySavedSearches = service.savedSearches({ owner: $C['USERNAME'], app: "SplunkEnterpriseSecuritySuite" });

                                                    mySavedSearches.fetch(function(err, mySavedSearches) {
                                                        var mySavedSearch = mySavedSearches.item(searchName)
                                                            // console.log("Do I have a Web Upload?", searchName, mySavedSearch)

                                                        // Modify the description and schedule the saved search
                                                        mySavedSearch.update({
                                                            "action.correlationsearch": 0,
                                                            "action.correlationsearch.enabled": 1,
                                                            'action.correlationsearch.label': currentSampleSearchLabel.replace(" - Demo", "").replace(" - Live", "").replace(" - Accelerated", ""),
                                                            'action.notable.param.security_domain': window.summary.domain.toLowerCase(),
                                                            'action.notable.param.rule_title': window.summary.name.toLowerCase(),
                                                            'action.correlationsearch.related_searches': '["Threat - ' + alertModal.model.alert.entry.content.get("name") + ' - Rule"]'
                                                                /*   DONE action.correlationsearch              = 0
                                                                     DONE action.correlationsearch.enabled      = 1
                                                                     DONE action.correlationsearch.label        = Threat Activity Detected
                                                                     DONE action.email.sendresults              = 0
                                                                     DONE action.notable                        = 1
                                                                     DONE action.notable.param.security_domain  = threat
                                                            ...
                                                            setTimeout(function() {
                                                                // console.log("\n...properties were modified...",obj)

                                                                var url = $C['SPLUNKD_PATH'] + "" + obj.qualifiedPath + "/acl"
                                                                var data = 'perms.read=*&perms.write=admin&owner=' + $C['USERNAME'] + '&sharing=app'
                                                                ...
                                                                $.ajax({
                                                                    ...
                                                                    data: data,
                                                                    success: function() {
                                                                        $("#ESCorrelationProcessing").html('ES Correlation Search Enabled! We recommend <a href="' + Splunk.util.make_full_url('/app/SplunkEnterpriseSecuritySuite/correlation_search_edit?search=' + searchName) + '">you click here</a> to continue editing the Notable Event to customize the display fields.')
                                                                    },
                                                                    error: function() {
                                                                        $("#ESCorrelationProcessing").html('Setting ES Correlation Search permissions failed! Look for your search in the ES App Saved Searches (not ES Content Management). Once you change the app permissions to share with App or All Apps, it will be viewable (and configurable) in ES Content Management.')
                                                                    }
                                                                })
                                                            }, 4000)
                                                        });
                                                    ...
                                                    // $(".modal:visible").find(".modal-body")
                                                    // Large_Web_Upload_try2


                                                }, 5000)

                                            })
                                            alertModal.model.alert.entry.content.set('action.notable', 1);
                                            alertModal.model.alert.entry.content.set('action.correlationsearch', 0);
                                            alertModal.model.alert.entry.content.set('action.correlationsearch.enable', 1);
                                            alertModal.model.alert.entry.content.set('action.correlationsearch.label', currentSampleSearchLabel.replace(" - Demo", "").replace(" - Live", "").replace(" - Accelerated", ""));
                                            alertModal.model.alert.entry.content.set('action.notable.param.security_domain', window.summary.domain.toLowerCase());
                                            alertModal.model.alert.entry.content.set('action.notable.param.rule_title', window.summary.name.toLowerCase());
                                            alertModal.model.alert.entry.content.set('action.correlationsearch.related_searches', '["Threat - ' + alertModal.model.alert.entry.content.get("name") + ' - Rule"]');
                                            /*   DONE action.correlationsearch              = 0
                                                 DONE action.correlationsearch.enabled      = 1
                                                 DONE action.correlationsearch.label        = Threat Activity Detected
                                                 DONE action.email.sendresults              = 0
                                    ...
                                    if (typeof window.actions.createRisk != "undefined" && window.actions.createRisk == 1) {
                                    ...
                                    alertModal.model.alert.on("saveSuccess", function(a, b, c) { window.saveSearchComplete_Deferral.resolve(a, b, c) })

                                    waitForEl(alertModal.el, function() {
                                        setTimeout(function() {
                                            ...
                                            // console.log("WWCD", currentSampleSearch)
                                            alertModal.model.alert.workingTimeRange.set("earliest", "-30d@d")
                                            alertModal.model.alert.workingTimeRange.set("latest", "@d")
                                            if (typeof(splunkjs.mvc.Components.getInstance("submitted").attributes["lookupToken"]) != "undefined" && splunkjs.mvc.Components.getInstance("submitted").attributes["lookupToken"].indexOf(".csv") > 0) {
                                    ...
                                    setTimeout(function() {
                                        $(".alert-save-as").find("a:contains(Save)").click(function() {
                                            require(["components/data/sendTelemetry", 'json!' + $C['SPLUNKD_PATH'] + '/servicesNS/nobody/Splunk_Security_Essentials/storage/collections/data/sse_app_config'], function(Telemetry, appConfig) {
                                                let record = { "status": "scheduleAlertCompleted", "searchName": currentSampleSearch.label }
                                                for(let i = 0; i < appConfig.length; i++){
                                                    if(appConfig[i].param == "demoMode" && appConfig[i].value == "true"){
                                                         record.demoMode = true
                                                    }
        ...
        function getFullSearchQueryArray() {
            ...
            let tokensToReplace = ["outlierValueTracked1Token", "outlierValueTracked2Token"]
            ...
            if (baseSearchString != null && outlierResultsSearchQuery != null) {
                ...
                let desiredTokenConfig = {}
                for(let i = 0; i < summary.examples.length; i++){
                    if(summary.examples[i].name == currentSampleSearchLabel){
                        desiredTokenConfig = summary.examples[i].showcase
                    }
                }
                for(let g = 0; g < tokensToReplace.length; g++){
                    if(desiredTokenConfig[ tokensToReplace[g].replace(/Token$/, "") ]){
                        if (typeof Forms.getToken(tokensToReplace[g])=='undefined') {
                            for(let i = 0; i < fullSearchQueryArray.length; i++){
                                let regex = new RegExp("\\$" + tokensToReplace[g] + "(.s)*\\$", "g");
                                if(fullSearchQueryArray[i].match(regex)){
                                    fullSearchQueryArray[i] = fullSearchQueryArray[i].replace(regex, desiredTokenConfig[ tokensToReplace[g].replace(/Token$/, "") ])
                                }
                            }
                        }
                    }
        ...
        function getFullSearchQueryComments() {
            return [null].concat(outlierResultsSearchSettings[getCurrentSearchName()].description, ['If the earliest time we have seen that value was within the last day, that means the first time we\'ve ever seen it just happened, and it qualifies as anomalous.']);
        }
        ...
        var updateForm = function updateForm(newIsRunningValue) {
            ...
            if (isRunning) {
                ...
                $("button:contains(Line-by-Line SPL Documentation)")[0].disabled = false
                $("button:contains(Line-by-Line SPL Documentation)").first().unbind("click")
                $("button:contains(Line-by-Line SPL Documentation)").first().click(function() { window.dvtestLaunchQuery() });
        ...
        function setCurrentSampleSearch(sampleSearch) {
            // console.log("hello 6.0", currentSampleSearch)
            var oldDataSetting = false
            if (sampleSearch.label.indexOf(" - Demo") > 0) {
                oldDataSetting = true
            }
            currentSampleSearch = _.extend({}, {
                useCurrentPoint: oldDataSetting
            }, sampleSearch);
            // console.log("Hello 6", sampleSearch, currentSampleSearch, currentPointCheckboxControl.val(), splunkjs.mvc.Components.getInstance("submitted").attributes.useCurrentPointToken)
            ...
            // console.log("Hello 6.1", sampleSearch, currentSampleSearch, currentPointCheckboxControl.val(), splunkjs.mvc.Components.getInstance("submitted").attributes.useCurrentPointToken)
            ...
            if (typeof currentSampleSearch.prereqs !== 'undefined') {
                ...
                ProcessSummaryUI.runPreReqs(currentSampleSearch.prereqs)
        ...
        var ModifiedLaunchQuerySPL = function(modalName, modalTitle, vizQueryArray, vizQueryComments, baseTimerange, vizOptions) {
            ...
            require(["components/data/sendTelemetry", 'json!' + $C['SPLUNKD_PATH'] + '/servicesNS/nobody/Splunk_Security_Essentials/storage/collections/data/sse_app_config'], function(Telemetry, appConfig) {
                let record = { "status": "SPLViewed", "name": currentSampleSearch.label }
                for(let i = 0; i < appConfig.length; i++){
                    if(appConfig[i].param == "demoMode" && appConfig[i].value == "true"){
                         record.demoMode = true
                    }
        ...
        Forms.setToken('useCurrentPointToken', true);
            ...
            PromptForLookupModal.footer.append($('<button>').addClass('mlts-modal-submit').attr({
                    ...
                    GetTheLookupNameModal.footer.append($('<button>').addClass('mlts-modal-submit').attr({
                            ...
                            CreateTheLookupModal.body.addClass('mlts-modal-form-inline')
                                .append($('<p>Creating lookup....<br/><br/><div id="StatusMessages" style="margin-left:0px;display:block;"></div><div style="margin-left:0px;display:block;" id="CreateLookupStatus"><center><img src="' + Splunk.util.make_full_url('/static/app/Splunk_Security_Essentials/images/general_images/loader.gif') + '"></center></div></p>'));
                            ...
                            CreateLookup(window.resultant_stanza, function() {
                                ...
                                document.getElementById("CreateLookupStatus").innerHTML = "<img src=\"" + Splunk.util.make_full_url("/static//app/Splunk_Security_Essentials/images/general_images/ok_ico.gif") + "\">";
                    ...
                    CheckForAvailable.on('search:error', function(properties) {
                        document.getElementById("resultant_available").innerHTML = "<img title=\"Error\" src=\"" + Splunk.util.make_full_url("/static//app/Splunk_Security_Essentials/images/general_images/err_ico.gif") + "\">";
                        $(".mlts-modal-submit:contains(Create)").first()[0].disabled = true
                    ...
                    CheckForAvailable.on('search:done', function(properties) {
                        ...
                        if (CheckForAvailable.attributes.data.resultCount == 0) {
                            document.getElementById("resultant_available").innerHTML = "<img src=\"" + Splunk.util.make_full_url("/static//app/Splunk_Security_Essentials/images/general_images/ok_ico.gif") + "\">";
                        ...
                        CheckForAvailableResults.on("data", function(properties) {
                            ...
                            if (data[0].count == 0) {
                                document.getElementById("resultant_available").innerHTML = "<img title=\"Error\" src=\"" + Splunk.util.make_full_url("/static//app/Splunk_Security_Essentials/images/general_images/ok_ico.gif") + "\">";
                                document.getElementById("resultant_text").innerHTML = "Click Next to create this lookup.";
                    ...
                    var LookupNameControl = function() {
                        ...
                        LookupNameControl.on('change', function(value) {
                            document.getElementById("resultant_available").innerHTML = '<img src="' + Splunk.util.make_full_url('/static/app/Splunk_Security_Essentials/images/general_images/loader.gif') + '" />';
                            $(".mlts-modal-submit:contains(Create)").first()[0].disabled = true
                            if (value.indexOf(".") > 0) {
        ...
        function CheckData(myDeferred, lookupName) {
            ...
            if (parseInt(document.getElementById("StatusCounter").innerHTML) == 2) {
                document.getElementById("StatusMessages").innerHTML = document.getElementById("StatusMessages").innerHTML.replace(/Try .*/, "<br/>Usually the fastest thing to get Splunk to recognize the lookup is for you to <a target=\"_blank\" href=\"" + Splunk.util.make_full_url("/app/Splunk_Security_Essentials/search?q=%7C%20inputlookup%20" + lookupName + "%20%7C%20inputlookup%20append%3Dt%20" + lookupName + ".csv&earliest=%40d&latest=now") + "\"><b>click this link</b></a>.<br /><br />Try <span id=\"StatusCounter\">2</span>...<br/>")
            }
