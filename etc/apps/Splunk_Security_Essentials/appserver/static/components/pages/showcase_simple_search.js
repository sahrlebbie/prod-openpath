...
require(["jquery",
    ...
    function($,
            ...
            (function setupOutliersVizSearch() {
                ...
                function scheduleAlert() {
                    ...
                    require(["components/data/sendTelemetry", 'json!' + $C['SPLUNKD_PATH'] + '/servicesNS/nobody/Splunk_Security_Essentials/storage/collections/data/sse_app_config'], function(Telemetry, appConfig) {
                        let record = { "status": "scheduleAlertStarted", "searchName": window.currentSampleSearch.label }
                                ...
                                if (isValid) {
                                    ...
                                    setTimeout(function() {
                                        $(".alert-save-as").find("a:contains(Save)").click(function() {
                                            require(["components/data/sendTelemetry", 'json!' + $C['SPLUNKD_PATH'] + '/servicesNS/nobody/Splunk_Security_Essentials/storage/collections/data/sse_app_config'], function(Telemetry, appConfig) {
                                                let record = { "status": "scheduleAlertCompleted", "searchName": window.currentSampleSearch.label }
                                                for(let i = 0; i < appConfig.length; i++){
                                                    if(appConfig[i].param == "demoMode" && appConfig[i].value == "true"){
                                                         record.demoMode = true
                                                    }
        ...
        function getFullSearchQueryComments() {
            return [null].concat(outlierResultsSearchSettings[getCurrentSearchName()].description);
        }
        ...
        var updateForm = function updateForm(newIsRunningValue) {
            ...
            if (isRunning) {
                ...
                if($("#primary-line-by-line-spl-docs").length == 0){

                    $("#assistantControlsFooter").append($('<button id="primary-line-by-line-spl-docs" class="btn">Line-by-Line SPL Documentation</button>').click(function(){
                        window.dvtestLaunchQuery()
                            ...
                            for (var example in summary.examples) {
                                if (summary.examples[example].name == sampleSearch.label) {
                                    window.summary = summary
                                        // console.log("Here are my passed parameters", summary, sampleSearch, ShowcaseInfo, showcaseName)
        ...
        var ModifiedLaunchQuerySPL = function(modalName, modalTitle, vizQueryArray, vizQueryComments, baseTimerange, vizOptions) {
            ...
            require(["components/data/sendTelemetry", 'json!' + $C['SPLUNKD_PATH'] + '/servicesNS/nobody/Splunk_Security_Essentials/storage/collections/data/sse_app_config'], function(Telemetry, appConfig) {
                let record = { "status": "SPLViewed", "searchName": window.currentSampleSearch.label }
                for(let i = 0; i < appConfig.length; i++){
                    if(appConfig[i].param == "demoMode" && appConfig[i].value == "true"){
                         record.demoMode = true
                    }
