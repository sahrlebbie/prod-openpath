...
window.Viz = []
localStorage["splunk-security-essentials-maxconcurrentsearches"] = localStorage["splunk-security-essentials-maxconcurrentsearches"] || 5
...
require([
    ...
    function(
        ...
        function KickItOff() {
            ...
            for (var item in window.datacheck) {
                if (item.indexOf("-") == -1 && item.indexOf("t") == -1) {
                    $("#" + item).html("<img title=\"Loading\" src=\"" + Splunk.util.make_full_url("/static//app/Splunk_Security_Essentials/images/general_images/loader.gif") + "\">");
            ...
            setTimeout(function() {
                ...
                $("#KickItOffButton").html('<i class="icon-cancel" /> Cancel Searches')
        ...
        $.getJSON('/static/app/Splunk_Security_Essentials/components/data/sampleSearches/showcase_first_seen_demo.json', function(data) {
            items = Object.assign(items, data)
        })
        $.getJSON('/static/app/Splunk_Security_Essentials/components/data/sampleSearches/showcase_standard_deviation.json', function(data) {
            items = Object.assign(items, data)
        })
        $.getJSON('/static/app/Splunk_Security_Essentials/components/data/sampleSearches/showcase_simple_search.json', function(data) {
            items = Object.assign(items, data)
        })
        ...
        $.getJSON($C['SPLUNKD_PATH'] + '/services/SSEShowcaseInfo?locale=' + window.localeString, function(data) {
            for(let summary in data.summaries){
                if(data.summaries[summary].split_multiple_examples){
                    for(let i = 0; i < data.summaries[summary].examples.length; i++){
                        if(data.summaries[summary].examples[i].live_split_eligible){
                            
                            let newId = data.summaries[summary].id + "_VERSION" + i
                            let newSummary = JSON.parse(JSON.stringify(data.summaries[summary]))
                            newSummary.name += " (" + data.summaries[summary].examples[i].label + ")";
                            newSummary.id = newId;
                            for(let g = 0; g < newSummary.examples.length; g++){
                                if(newSummary.examples[g].is_live_version){
                                    delete newSummary.examples[g].is_live_version
                                }
                            }
                            newSummary.examples[i].is_live_version = true;
                            newSummary.dashboard = newSummary.dashboard.replace(/dataset=.*/, "dataset=" + newSummary.examples[i].name)
                            data.summaries[newId] = newSummary;
                            data.roles.default.summaries.push(newId)
                        }
                    }
                    delete data.summaries[summary]
                    data.roles.default.summaries.splice(data.roles.default.summaries.indexOf(summary), 1)
                }
            }
            for(let summary in data.summaries){
                if(data.summaries[summary].examples){
                    for(let i = 0; i < data.summaries[summary].examples.length; i++){
                        if(data.summaries[summary].examples[i].is_live_version){
                            data.summaries[summary].examples[i].label = "Live Data"
                        }
                    }
                }
            }
            ...
            for (var i = 0; i < ShowcaseInfo.roles.default.summaries.length; i++) {
                ...
                if (summary.dashboard.indexOf("=") > 0) {
                    example = summary.dashboard.substr(summary.dashboard.indexOf("=") + 1).replace(/ \-.*/, "")
                }
                ...
                if (typeof example != "undefined") {
                    ...
                    if (typeof summary.examples != "undefined") {
                        ...
                        summary.examples.forEach(function(example) {
                            ...
                            window.SearchToShowcase[example.name.replace(/ \- .*/, "")] = summary_name
                    ...
                    if ((accel != "" && typeof items[accel] != "undefined" && typeof items[accel].prereqs == "object")) {
                    ...
                    if ((demo != "" && typeof items[demo] != "undefined" && typeof items[demo].prereqs == "object") || (live != "" && typeof items[live] != "undefined" && typeof items[live].prereqs == "object") || (accel != "" && typeof items[accel] != "undefined" && typeof items[accel].prereqs == "object")) {
                        ...
                        if (typeof summary.bookmark_status != "undefined") {
                            switch (summary.bookmark_status) {
                                case "none":
                                    bookmarkWidget = '<img class="bookmarkIcon" title="Bookmark this content to look up later" data-showcase="' + summary.name + '" src="' + Splunk.util.make_full_url("/static/app/Splunk_Security_Essentials/images/general_images/nobookmark.png") + '" onclick=\'createWishlistBox(this, "' + summary.name + '"); return false;\' />'
                                    enabledWidget = $('<img class="enabledIcon" title="Mark that the content is already enabled" onclick="markContentEnabled(this);" data-showcase="' + summary.name + '" src="' + Splunk.util.make_full_url("/static/app/Splunk_Security_Essentials/images/general_images/notenabled.png") + '"  />')
                                    break;
                                case "bookmarked":
                                    bookmarkWidget = '<i class="icon-bookmark" data-showcase="' + summary.name + '" title="Bookmarked" onclick=\'createWishlistBox(this, "' + summary.name + '"); return false;\' />'
                                    break;
                                case "needData":
                                    bookmarkWidget = '<i class="icon-bookmark" data-showcase="' + summary.name + '" title="Bookmarked" onclick=\'createWishlistBox(this, "' + summary.name + '"); return false;\' />'
                                    break;
                                ...
                                case "successfullyImplemented":
                                    bookmarkWidget = '<i class="icon-check" data-showcase="' + summary.name + '" title="Implemented" onclick=\'createWishlistBox(this, "' + summary.name + '"); return false;\' />'
                                    break;
                        ...
                        window.markContentEnabled = function(obj) {
                            let showcaseId = $(obj).closest("tr").attr("data-showcaseId")
                            let target = $(obj)
                            let name = target.attr("data-showcase")
                                // console.log("Marking conte")
                            if (target.attr("title") == "Mark that the content is already enabled") {
                                setbookmark_status(name, showcaseId, "successfullyImplemented")
                                target.closest(".bookmarkIcons").html('<i class="icon-check" data-showcase="' + name + '" title="Implemented" onclick=\'createWishlistBox(this, "' + name + '"); return false;\'  />')
                        ...
                        window.createWishlistBox = function(obj, name) {
                            ...
                            if ($(obj).is("img")) {
                                ...
                                container.html('<i class="icon-bookmark" data-showcase="' + name + '" title="Bookmarked" onclick=\'createWishlistBox(this, "' + name + '"); return false;\'  />')
                                ...
                                let bookmarkWidget = $('<img class="bookmarkIcon" title="Bookmark this content to look up later" data-showcase="' + name + '" src="' + Splunk.util.make_full_url("/static/app/Splunk_Security_Essentials/images/general_images/nobookmark.png") + '" onclick=\'createWishlistBox(this, "' + name + '"); return false;\' />')
                                let enabledWidget = $('<img class="enabledIcon" title="Mark that the content is already enabled" onclick="markContentEnabled(this);" data-showcase="' + name + '" src="' + Splunk.util.make_full_url("/static/app/Splunk_Security_Essentials/images/general_images/notenabled.png") + '" />')
                                let container = $(obj).closest(".bookmarkIcons")
                                container.html("")
                                container.append(bookmarkWidget, enabledWidget)
                        ...
                        let Icons = $('<div class="bookmarkIcons">').append($(bookmarkWidget), enabledWidget)
                            // console.log("For ", summary, Icons.html())
                        $("#main_table_body").append("<tr data-showcaseid=\"" + summary.id + "\" id=\"row-" + toHex(example) + "\" class=\"dvbanner\"><td class=\"tableexpand\" class=\"downarrow\" id=\"expand-" + toHex(example) + "\"><a href=\"#\" onclick=\"doToggle('" + toHex(example) + "'); return false;\"><i class=\"icon-chevron-right\" /></a></td><td class=\"name\"><a href=\"#\" onclick=\"doToggle('" + toHex(example) + "'); return false;\">" + summary.name + "</a>&nbsp;" + Icons[0].outerHTML + "</td><td class=\"tabledemo\" id=\"" + toHex(example + "-demo") + "\">" + demodisplay + "</td><td class=\"tablelive\" id=\"" + toHex(example + "-live") + "\">" + livedisplay + "</td><td class=\"tableaccel\" id=\"" + toHex(example + "-accel") + "\">" + acceldisplay + "</td></tr>")
                    ...
                    if (demo != "" && typeof items[demo] != "undefined") {
                        if (typeof items[demo].prereqs != "undefined" && items[demo].prereqs.length > 0) {
                            ...
                            demo = "<img style=\"height:22px; width:20px;\" src=\"" + Splunk.util.make_full_url("/static/app/Splunk_Security_Essentials/images/general_images/loader.gif") + "\">"
                        } else {
                            demo = ""
                        }
                    }
                    if (live != "" && typeof items[live] != "undefined") {
                        if (items[live].eventtypeInference && items[live].eventtypeInference != "") {
                            window.inferences[toHex(example + "-live")] = items[live].eventtypeInference
                        }
                        if (typeof items[live].prereqs != "undefined" && items[live].prereqs.length > 0) {
                            ...
                            live = "<img style=\"height:22px; width:20px;\" src=\"" + Splunk.util.make_full_url("/static/app/Splunk_Security_Essentials/images/general_images/loader.gif") + "\">"
                        } else {
                            live = ""
                        }
                    }
                    if (accel != "" && typeof items[accel] != "undefined") {
                        if (items[accel].eventtypeInference && items[accel].eventtypeInference != "") {
                            window.inferences[toHex(example + "-accel")] = items[accel].eventtypeInference
                        }
                        if (typeof items[accel].prereqs != "undefined" && items[accel].prereqs.length > 0) {
                            ...
                            accel = "<img style=\"height:22px; width:20px;\" src=\"" + Splunk.util.make_full_url("/static/app/Splunk_Security_Essentials/images/general_images/loader.gif") + "\">"
                        } else {
                            accel = ""
                        }
                    }
            ...
            $.ajax({
                url: $C['SPLUNKD_PATH'] + '/servicesNS/nobody/Splunk_Security_Essentials/storage/collections/data/data_source_check/',
                type: 'GET',
                contentType: "application/json",
                async: false,
                success: function(returneddata) {
                    if (returneddata.length > 1) {
                        window.KVStoreHistory = returneddata
                            // Print the Button
                        $("#placeholderForRestore")[0].outerHTML = '<button type="button" id="pullKVStoreButton" class="btn btn-primary"><i class="icon-rotate-counter"></i> Retrieve Last Result</button>'
                        $("#pullKVStoreButton").click(function() {
                            RetrieveSearchKVstore()
                            $("#KickItOffButton").remove()
                            $("#pullKVStoreButton").addClass("disabled").attr("disabled", "disabled")
                    ...
                    if (typeof window.datacheck[searchHex] == "undefined") {
                        ...
                        window.datacheck[searchHex].mainSearch.on('search:start', function(properties) {
                            for (var g = 0; g < window.datacheck[properties.content.request.label].registerResults.length; g++) {
                                ...
                                document.getElementById("data_check_test-" + element + "-" + myCheckNum).innerHTML = "<img title=\"Loading\" src=\"" + Splunk.util.make_full_url("/static//app/Splunk_Security_Essentials/images/general_images/loader.gif") + "\">";
                                if (window.location.href.indexOf("debug=true") >= 0) {
                        ...
                        window.datacheck[searchHex].mainSearch.on('search:error', function(properties) {
                            for (var g = 0; g < window.datacheck[properties.content.request.label].registerResults.length; g++) {
                                ...
                                var element = matches[1]
                                var myCheckNum = matches[2]
                                document.getElementById("data_check_test-" + element + "-" + myCheckNum).innerHTML = "<img title=\"Error\" src=\"" + Splunk.util.make_full_url("/static//app/Splunk_Security_Essentials/images/general_images/err_ico.gif") + "\">";
                                if (window.location.href.indexOf("debug=true") >= 0) {
                                    param = "#data_check_test-" + element + "-" + myCheckNum
                                    $(param).append(" <a href=\"#\" onclick=\"runDebug('" + $(param).attr("id") + "'); return false\">(debug)</a>")
                                }
                        ...
                        window.datacheck[searchHex].mainSearch.on('search:done', function(properties) {
                            ...
                            window.datacheck[searchHex].myResults.on("data", function(properties) {
                                ...
                                for (var g = 0; g < window.datacheck[searchHex].registerResults.length; g++) {
                                    ...
                                    if (status == true) {
                                        document.getElementById("data_check_test-" + element + "-" + myCheckNum).innerHTML = "<img title=\"Success\" src=\"" + Splunk.util.make_full_url("/static//app/Splunk_Security_Essentials/images/general_images/ok_ico.gif") + "\">";
                                        if (window.location.href.indexOf("debug=true") >= 0) {
                                            param = "#data_check_test-" + element + "-" + myCheckNum
                                            $(param).append(" <a href=\"#\" onclick=\"runDebug('" + $(param).attr("id") + "'); return false\">(debug)</a>")
                                        }
    ...
    for (var g = 0; g < window.datacheck[searchHex].registerResults.length; g++) {
        if (typeof window.datacheck[searchHex].mainSearch.attributes.data != "undefined" && window.datacheck[searchHex].mainSearch.attributes.data.doneProgress == 1) {
            ...
            if (window.datacheck[searchHex].mainSearch.attributes.data.isFailed == 1 || window.datacheck[searchHex].mainSearch.attributes.data.resultCount == 0) {
                ...
                document.getElementById(window.datacheck[searchHex].registerResults[g]).innerHTML = "<img title=\"Error\" src=\"" + Splunk.util.make_full_url("/static//app/Splunk_Security_Essentials/images/general_images/err_ico.gif") + "\">";
...
function doDataCheck(element) {
    ...
    if ($("#" + element).find("img")[0]) {
        $("#" + element).html(popover + "<img title=\"" + title + "\" src=\"" + Splunk.util.make_full_url("/static/app/Splunk_Security_Essentials/images/general_images/" + status + "") + "\" /></a>")
        $("#" + element).find("a").popover()
    }
...
function pullVizForCompletedRows(parentElement, doUpdate, idofelement) {
    ...
    if (typeof window.result_tracking[parentElement.attr("id")] == "undefined") {
        window.result_tracking[parentElement.attr("id")] = { "accel": 0, "live": 0, "error": 0 }
    }
    if ((parentElement.find(".tableaccel").find("img").length && parentElement.find(".tableaccel").find("img").attr("src").match(/ok_ico.gif/)) &&
        ...
        if (doUpdate == true) {
            ...
            updateStatus(window.SearchToShowcase[fromHex(parentElement.attr("id").replace("row-", ""))], ShowcaseInfo['summaries'][window.SearchToShowcase[fromHex(parentElement.attr("id").replace("row-", ""))]]['name'], "accel")
        }
        addSearch(window.SearchToShowcase[fromHex(parentElement.attr("id").replace("row-", ""))], "Accelerated Data", true)
    }
    if (((parentElement.find(".tableaccel").find("img").length && parentElement.find(".tableaccel").find("img").attr("src").match(/err_ico.gif/)) ||
            parentElement.find(".tableaccel").find("a:contains(N/A)").length) &&
        (parentElement.find(".tablelive").find("img").length && parentElement.find(".tablelive").find("img").attr("src").match(/ok_ico.gif/)) &&
        ...
        if (doUpdate == true) {
            gotInferenceUpdate(idofelement)
            updateStatus(window.SearchToShowcase[fromHex(parentElement.attr("id").replace("row-", ""))], ShowcaseInfo['summaries'][window.SearchToShowcase[fromHex(parentElement.attr("id").replace("row-", ""))]]['name'], "live")
        }
        addSearch(window.SearchToShowcase[fromHex(parentElement.attr("id").replace("row-", ""))], "Live Data", true)
    }
    if (((parentElement.find(".tableaccel").find("img").length && parentElement.find(".tableaccel").find("img").attr("src").match(/err_ico.gif/)) ||
            parentElement.find(".tableaccel").find("a:contains(N/A)").length) &&
        (parentElement.find(".tablelive").find("img").length && parentElement.find(".tablelive").find("img").attr("src").match(/err_ico.gif/)) &&
        ...
        addSearch(window.SearchToShowcase[fromHex(parentElement.attr("id").replace("row-", ""))], "Live Data", false)
    }

    if ($(parentElement).find("img[title=Loading]").length == 0 && status_to_post != "") {
...
function runDebug(element) {
    ...
    confirmVizAlertModal.footer.append($('<button>').attr({
            type: 'button',
            'data-dismiss': 'modal'
...
function gotInferenceUpdate(elementId) {
    ...
    if (typeof window.inferences[elementId] != "undefined") {
        ...
        for (let i = 0; i < eventtypes.length; i++) {
            ...
            let newStatus = ""
            if (typeof window.publishedInferences[eventtype] == "undefined" && $("#" + elementId).find("img").length > 0 && $("#" + elementId).find("img").attr("src").indexOf("ok_ico") >= 0 || (typeof window.publishedInferences[eventtype] != "undefined" && window.publishedInferences[eventtype] != 100 && $("#" + elementId).find("img").length > 0 && $("#" + elementId).find("img").attr("src").indexOf("ok_ico") >= 0)) {
                newStatus = 100
            } else if (typeof window.publishedInferences[eventtype] == "undefined" && $("#" + elementId).find("img").length > 0 && $("#" + elementId).find("img").attr("src").indexOf("err_ico") >= 0) {
                newStatus = 0
            }
            ...
            if (newStatus !== "") {
                ...
                require([Splunk.util.make_full_url("/static/app/Splunk_Security_Essentials/components/data/sendTelemetry.js")], function(Telemetry) {
                    Telemetry.SendTelemetryToSplunk("DataStatusChange", { "status": newStatus, "selectionType": "data_source_check", "category": eventtype })
                })
            }
        }
    }
...
function updateStatus(element, elementName, status) {
    ...
    $.ajax({
        url: $C['SPLUNKD_PATH'] + '/servicesNS/nobody/Splunk_Security_Essentials/storage/collections/data/data_source_check_outputs/',
        type: 'POST',
        contentType: "application/json",
        async: false,
        data: JSON.stringify(record),
        ...
        error: function(xhr, textStatus, error) {
            $.ajax({
                url: $C['SPLUNKD_PATH'] + '/servicesNS/nobody/Splunk_Security_Essentials/storage/collections/data/data_source_check_outputs/' + element,
                type: 'POST',
                contentType: "application/json",
                async: false,
                data: JSON.stringify(record),
...
function postDashboard(dashboardName, XML) {
    ...
    dashboardName = dashboardName.replace(/ /g, "_").replace(/[^a-zA-Z0-9_\-]/g, "");
    var record = {
        "eai:data": XML,
        "name": dashboardName
    }
    ...
    $.ajax({
        url: $C['SPLUNKD_PATH'] + "/servicesNS/nobody/Splunk_Security_Essentials/data/ui/views/",
        data: record,
        contentType: "application/json",
        type: 'POST',
        ...
        error: function(data, error) {
            ...
            $.ajax({
                url: $C['SPLUNKD_PATH'] + "/servicesNS/nobody/Splunk_Security_Essentials/data/ui/views/" + dashboardName,
                data: record,
                contentType: "application/json",
                type: 'POST',
                success: function(data) {
...
function GenerateDashboardModals() {
    ...
    require(['jquery', 'app/Splunk_Security_Essentials/components/controls/Modal'], function($, Modal) {

        if (window.SearchesInProgress.length == 0 && window.SearchesComplete.length == 0) {
            ...
            myTestModal.body.append("<p>Please click the Run Searches button on the upper right, so the system can check what visualizations are available to with your data.</p>")

            myTestModal.footer.append($('<button>').addClass('mlts-modal-submit').attr({
                type: 'button',
                'data-dismiss': 'modal'
        ...
        if (window.SearchesInProgress.length > 0 || window.SearchesInQueue.length > 0) {
            ...
            myTestModal.body.append("<p>Please wait until searches complete before running.</p>")
            ...
            require([
                    "jquery",
                    Splunk.util.make_full_url("/static/app/Splunk_Security_Essentials/vendor/jquery.md5/jquery.md5.js")
                ],
                function(
                    $
                ) {
                    window.Viz.forEach(function(singleViz) {
                        if (singleViz.header != "Broken") {
                            ...
                            if (typeof AllViz[singleViz.dashboard][singleViz.header] == "undefined") {
                                AllViz[singleViz.dashboard][singleViz.header] = []
                            }
                            var myLocal = template.clone()
                            myLocal.css("margin", "5px").css("padding-left", "40px").css("padding-right", "40px").css("max-width", "400px").css("white-space", "normal").css("position", "relative")
                            if (typeof singleViz.description == "undefined" || singleViz.description == "")
                                myLocal.html("<i style=\"float: left; width: 20px; height: 20px; color: green; border: solid gray 1px; position: absolute; left: 5px; \"></i><span class=\"panelTitle\">" + singleViz.title + "</span>")
                            else
                                myLocal.html("<i style=\"float: left; width: 20px; height: 20px; color: green; border: solid gray 1px; position: absolute; left: 5px; \"></i><span class=\"panelTitle\">" + singleViz.title + "</span><hr style=\"margin-top: 3px; margin-bottom: 3px; margin-left: auto; margin-right: auto; width: 50%\"/><span class=\"panelDescription\">" + singleViz.description + "</span>")
                            if (singleViz.status == false) {
                                myLocal.attr("disabled", "disabled").addClass("disabled")
                                myLocal.find("i").css("display", "none")
                                myLocal.css("padding-left", "20px").css("padding-right", "20px")
                                var links = []
                                window.ShowcaseInfo['summaries'][singleViz.sourceSummary].datasource.split("|").forEach(function(source) {
                                    links.push('<a href="data_source?datasource=' + source.replace(/ /g, "%20") + '" target="_blank">' + source + "</a>")
                                })

                                myLocal.append("<hr style=\"margin-top: 3px; margin-bottom: 3px; margin-left: auto; margin-right: auto; width: 50%\"/><span class=\"panelDescription\">Missing Data Sources:<br/>" + links.join(", ") + "</span>")
                            } else {
                                if (typeof singleViz.recommended == "undefined" || singleViz.recommended == true) {
                                ...
                                myLocal.click(function(evt) {
                                    ...
                                    CalculateStatusForDashboardHeader(obj.closest(".dashboardViz"))
                                })
                            }
                            AllViz[singleViz.dashboard][singleViz.header].push(myLocal)

                        }
                    })

                    var vizOptionsModal = new Modal('VizOptionPanel', {
                        title: 'Available Visualizations',
                        destroyOnHide: true,
                        type: 'wide'
                    });
                    vizOptionsModal.$el.css("width", "90%").css("margin-left", "-45%").css("height", (window.innerHeight - 50) + "px")

                    vizOptionsModal.body.append("<p>Below, please find the list of available dashboard elements. We have taken the liberty of enabling several dashboard panels that are frequently requested, but you can click on or off any panels based on your needs. To ensure a good user experience, we've disabled any panels for which you do not have the required data.</p><div id=\"viz_elements\"></div> ").css("overflow", "scroll")

                    vizOptionsModal.footer.append($('<button>').attr({
                        type: 'button',
                        'data-dismiss': 'modal'
                        ...
                        type: 'button'
                    }).addClass('btn btn-primary').text('Create Dashboards').on('click', function() {

                        // Not taking any action on Close
                        var newDashboards = $("<div>").html("<p>Dashboard(s) complete, and added into the navigation! (You'll see them on the next page you open up.) You can also click below to open them.</p>")
                        var listOfDashboards = []
                        $(".dashboardViz:has(button.viz.active):has(.dashboardEnabled:checked)").each(function(num, obj) {

                            var dashboardViz = $(obj)
                            var simpleXML = "<form>\n\t<label>" + dashboardViz.find("h2").text() + "</label>\nBASESEARCHPLACEHOLDER"
                            simpleXML += "\n\t<fieldset submitButton=\"false\">\n\t\t<input type=\"time\" token=\"timerange\">\n\t\t\t<label></label>\n\t\t\t<default>\n\t\t\t\t<earliest>-7d@d</earliest>\n\t\t\t\t<latest>now</latest>\n\t\t\t</default>\n\t\t</input>\n\t</fieldset>"
                            var baseSearch = new Object()
                            dashboardViz.find(".headerViz:has(button.viz.active)").each(function(num, obj) {
                                var headerViz = $(obj)
                                var row = "\n\t<row>\n\t\t<panel>\n\t\t\t<title>" + headerViz.find("h3").text() + "</title>\n\t\t\t<html>\n\t\t\t</html>\n\t\t</panel>\n\t</row>\n\t<row>"
                                headerViz.find("button.viz.active").each(function(num, obj) {

                                    var button = $(obj)
                                    row += "\n\t\t<panel>\n\t\t\t<title>" + button.find(".panelTitle").text() + "</title>"
                                    if (button.find(".panelDescription").length >= 1) {
                                        row += "\n\t\t\t<description>" + button.find(".panelDescription").text() + "</description>"
                                    }
                                    window.Viz.forEach(function(singleViz) {
                                        if (singleViz.title == button.find(".panelTitle").text()) {
                                            var BaseSearchRef = ""
                                                //                 console.log("Analyzing singleViz", singleViz)
                                            if (typeof singleViz.basesearch != "undefined") {
                                                BaseSearchRef = " base=\"search" + $.md5(singleViz.basesearch) + "\""
                                                baseSearch[$.md5(singleViz.basesearch)] = "\n\t<search id=\"search" + $.md5(singleViz.basesearch) + "\">\n\t\t<query>" + splunkEncode(singleViz.basesearch) + "</query>\n\t\t<earliest>$timerange.earliest$</earliest>\n\t\t<latest>$timerange.latest$</latest>\n\t</search>"
                                            }
                                            row += "\n\t\t\t<" + singleViz.vizType + ">"
                                            row += "\n\t\t\t\t<search" + BaseSearchRef + ">"
                                            row += "\n\t\t\t\t\t<query>" + splunkEncode(singleViz.search) + "</query>"
                                            if (typeof singleViz.basesearch == "undefined")
                                                row += "\n\t\t\t\t\t<earliest>$timerange.earliest$</earliest>\n\t\t\t\t\t<latest>$timerange.latest$</latest>\n\t"
                                            row += "\n\t\t\t\t</search>"
                                            for (var param in singleViz.vizParameters) {
                                                row += "\n\t\t\t\t<option name=\"" + param + "\">" + singleViz.vizParameters[param] + "</option>"
                                            }
                                            row += "\n\t\t\t</" + singleViz.vizType + ">"

                                            if (typeof singleViz.description != "undefined" && singleViz.description != "") {
                                                row += "\n\t\t\t<html><p>" + button.find(".panelDescription").text() + "</p></html>"
                                            }
                                ...
                                row += "\n\t</row>"
                                row = row.replace(/ChartElement/g, "chart").replace(/SingleElement/g, "single").replace(/MapElement/g, "map").replace(/TableElement/g, "table")
                                simpleXML += row
                            })
                            simpleXML += "\n</form>"
                            var allBaseSearches = ""
                            ...
                            simpleXML = simpleXML.replace(/BASESEARCHPLACEHOLDER/, allBaseSearches)
                            var officialDashboardName = postDashboard(dashboardViz.find("h2").text(), simpleXML)
                            newDashboards.append($("<p>").append($("<a>").attr("target", "_blank").attr("href", officialDashboardName).text(dashboardViz.find("h2").text())))
                        ...
                        var myCompletedModal = new Modal('completedModal', {
                        ...
                        myCompletedModal.footer.append($('<button>').attr({
                            type: 'button',
                            'data-dismiss': 'modal'
                    ...
                    for (var dashboard in AllViz) {
                        ...
                        var dashboardDiv = $('<div class="dashboardViz"></div>').append($("<div class=\"dashboardHeader\">").append($('<input type="checkbox" class="dashboardEnabled">'), $("<h2 class=\"dashboardName\">").text(dashboard), $('<div class="dashboardStatus">')))
                        dashboardDiv.find(".dashboardEnabled").css("margin-top", "0px").css("margin-right", "10px").click(function(evt) {
                            var dashboardDiv = $(evt.target).closest(".dashboardViz")
                            if (evt.target.checked)
                                dashboardDiv.find(".headerViz").css("display", "block")
                            else
                                dashboardDiv.find(".headerViz").css("display", "none")
                            CalculateStatusForDashboardHeader(dashboardDiv)
                            dashboardDiv.closest(".modal-body").css("max-height", (window.innerHeight - 175) + "px")
                        })
                        dashboardDiv.find(".dashboardName").css("display", "inline")
                        dashboardDiv.find(".dashboardStatus").css("float", "right")
                        dashboardDiv.css("border", "1.5px gray solid").css("border-radius", "15px").css("padding", "15px").css("margin-top", "5px").css("margin-bottom", "5px")
                        for (var header in AllViz[dashboard]) {
                            var headerDiv = $('<div class="headerViz" style="display: none;"></div>').append($("<h3>").text(header))
                            for (var i = 0; i < AllViz[dashboard][header].length; i++) {
                                headerDiv.append(AllViz[dashboard][header][i])
                            }
                            dashboardDiv.append(headerDiv)
                        }
                        CalculateStatusForDashboardHeader(dashboardDiv)
                        vizOptionsModal.body.find("#viz_elements").append(dashboardDiv)
                    }
                    vizOptionsModal.body.find("#viz_elements").append('<div><p><a href="#" id="loadDemoLink">Use Demo Datasets</a></p></div>')
...
function CalculateStatusForDashboardHeader(element) {
    ...
    element.find(".dashboardStatus").append($("<span><b>" + active + "</b> selected " + "<span style=\"color: gray\">|&nbsp;</span></span>"), $("<span>" + available + ' available <span style=\"color: gray\">|&nbsp;</span></span>'), $("<span>" + total + " total</span>"))
...
function CheckWhetherAnyDashboardsAreEnabled() {
    ...
    $(".dashboardStatus").each(function(num, obj) {
        count += parseInt($(obj).text().replace(/ selected.*/, ""))
    })
    if (count == 0) {
        $(".dashboardStatus").first().closest(".modal").find(".modal-footer").find(".btn-primary").addClass("disabled").attr("disabled", "disabled")
    } else {
        $(".dashboardStatus").first().closest(".modal").find(".modal-footer").find(".btn-primary").removeClass("disabled").removeAttr("disabled")
    }
}
...
function AddPostureDashboardsToNav(listOfDashboards) {
    ...
    if (dashboardStr != "") {
        ...
        $.ajax({
            ...
            type: 'GET',
            success: function(data) {
                var newNav = $(data).find("[name='eai:data']").text().replace(/<collection label="Posture Dashboards">[\n\s!-~]*?<\/collection>\n/, "").replace(/<\/nav>/, '  <collection label="Posture Dashboards">' + dashboardStr + '\n  </collection>\n</nav>')
...
function FinalizeSearches() {
    for (var obj in splunkjs.mvc.Components.attributes) {
        if (typeof splunkjs.mvc.Components.getInstance(obj).attributes != "undefined" && typeof splunkjs.mvc.Components.getInstance(obj).attributes.search != "undefined") {
...
function RecordStatusOfPreCheck() {
    // console.log("Kicking off RecordStatusOfPreCheck")
    ...
    $(".tableaccel,.tablelive,.tabledemo").each(function(num, obj) {
        ...
        if ($(obj).find("img").length > 0) {
            if ($(obj).find("img").attr("src").match(/ok_ico.gif/)) {
                status = "Good"
            } else if ($(obj).find("img").attr("src").match(/err_ico.gif/)) {
                status = "Error"
            } else if ($(obj).find("img").attr("src").match(/loader.gif/)) {
                status = "NotComplete"
            }
        } else if ($(obj).find("a:contains(N/A)").length > 0) {
            status = "NotApplicable"
        }
        if (status != "NotApplicable" && typeof status != "undefined" && status != "") {
            var searchId = obj.id;
            var searchName = fromHex(searchId).replace(/-live/, " - Live").replace(/-demo/, " - Demo").replace(/-accel/, " - Accelerated")

            var showcaseId = SearchToShowcase[searchName]
                //console.log("Here's our Status and ID", status, searchId, searchName, showcaseId)
            var showcaseName = ShowcaseInfo['summaries'][showcaseId].name
                //console.log("Here's our Status and ID", status, searchName, searchId, showcaseId, showcaseName)
            UpdateSearchKVstore(status, searchName, searchId, showcaseId, showcaseName)
        }

    })
}

function UpdateSearchKVstore(status, searchName, searchId, showcaseId, showcaseName) {
    ...
    $.ajax({
        url: $C['SPLUNKD_PATH'] + '/servicesNS/nobody/Splunk_Security_Essentials/storage/collections/data/data_source_check/?query={"_key": "' + searchId + '"}',
        type: 'GET',
        contentType: "application/json",
        async: false,
        success: function(returneddata) {
            if (returneddata.length == 0) {
                ...
                // Old
                $.ajax({
                    url: $C['SPLUNKD_PATH'] + '/servicesNS/nobody/Splunk_Security_Essentials/storage/collections/data/data_source_check/' + searchId,
                    type: 'POST',
                    contentType: "application/json",
                    async: false,
                    data: JSON.stringify(record),
...
function RetrieveSearchKVstore() {
    if (typeof window.KVStoreHistory != "undefined" && window.KVStoreHistory.length > 1) {
        var result = new Object()
        result["Good"] = '<a class="dvPopover" href="#" data-original-title="Success" data-toggle="popover" title="Success" data-trigger="hover" data-content="All of the prerequisite checks were complete for this search, so you should be able to run it in your environment. Click the expand icon on the right to find out more, or for a link to the search."><img title=\"Success\" src=\"' + Splunk.util.make_full_url("/static/app/Splunk_Security_Essentials/images/general_images/ok_ico.gif") + '\" /></a>'
        result["Error"] = '<a class="dvPopover" href="#" data-original-title="Error" data-toggle="popover" title="Error" data-trigger="hover" data-content="One of more of the prerequisite checks for this search failed. Click the expand icon on the right to find which failed, and how to remediate those."><img title=\"Error\" src=\"' + Splunk.util.make_full_url("/static/app/Splunk_Security_Essentials/images/general_images/err_ico.gif") + '\" /></a>'
        result["NotComplete"] = '<a class="dvPopover" href="#" data-original-title="Loading" data-toggle="popover" title="Loading" data-trigger="hover" data-content="One or more of the prerequisite checks are still running for this search. Click the expand icon on the right to find out more, or for a link to the search."><img title=\"Loading\" src=\"' + Splunk.util.make_full_url("/static/app/Splunk_Security_Essentials/images/general_images/loader.gif") + '\" /></a>'
        ...
        for (var i = 0; i < window.KVStoreHistory.length; i++) {
            ...
            $("#" + window.KVStoreHistory[i]['_key']).html(result[window.KVStoreHistory[i].status])
            var exampleName = ""
                //       console.log("Looking for the examples for", window.KVStoreHistory[i].showcaseId)
            if(window.ShowcaseInfo['summaries'][window.KVStoreHistory[i].showcaseId] && window.ShowcaseInfo['summaries'][window.KVStoreHistory[i].showcaseId].examples){
                for (var g = 0; g < window.ShowcaseInfo['summaries'][window.KVStoreHistory[i].showcaseId].examples.length; g++) {
                    if (window.ShowcaseInfo['summaries'][window.KVStoreHistory[i].showcaseId].examples[g].name == window.KVStoreHistory[i].searchName) {
                        exampleName = window.ShowcaseInfo['summaries'][window.KVStoreHistory[i].showcaseId].examples[g].label
                    }
                }
            }
            var status = false;
        ...
        $("td.tableaccel:empty").html('<a class="dvPopover" href="#" data-toggle="popover" title="Not Applicable" data-trigger="hover" data-content="We do not currently have a search that leverages the accelerated data.">' + "<span style=\"color: black;\" title=\"Not Applicable (no search present)\">N/A</span>" + '</a>')
        ...
        $('img[title="In Queue..."]').each(function(num, obj){ 
            let td = $(obj).parent();
            let link=$('<a href="#" title="Detailed status not provided when restoring from kvstore." click="return false;">?</a>')
            td.html(link)
                ...
                if (typeof examples != "undefined") {
                    ...
                    examples.forEach(function(example) {
                        ...
                        if (example.name.match(/ - Demo/)) {
                            stages[4]++;
                            if (typeof window.Searches[example.name] != "undefined" && typeof window.Searches[example.name].visualizations != "undefined" && window.Searches[example.name].visualizations.length >= 1) {
                                stages[5]++;
                                window.Searches[example.name].visualizations.forEach(function(viz) {
                                    stages[6]++;
                                    viz.status = true;
