    ...
    function ($,
        ...
        for (var SummaryName in ShowcaseInfo['summaries']) {
            if (ShowcaseInfo['summaries'][SummaryName].name == requestedShowcase) {
                ...
                if (chosenSummary.phantomPlaybooks) {
                    ...
                    let myPhantomBlock = $("<div>").append(Template.replace(/SHORTNAME/g, "phantomPlaybooks").replace("TITLE", "Phantom Playbooks") + "</td></tr></table>")

                    let phantomPlaybookText = ""

                    //phantomPlaybookText = "<h2>" + _("Phantom Playbooks").t() + "</h2>"
                    var tiles = $('<ul class="showcase-list"></ul>')
                    for (var i = 0; i < chosenSummary.phantomPlaybooks.length; i++) {
                        if (typeof ShowcaseInfo['summaries'][chosenSummary.phantomPlaybooks[i]] != "undefined")
                            tiles.append($("<li style=\"width: 230px; height: 320px\"></li>").append(BuildTile.build_tile(ShowcaseInfo['summaries'][chosenSummary.phantomPlaybooks[i]], true)))

                    }
                    phantomPlaybookText += '<ul class="showcase-list">' + tiles.html() + '</ul>'


                    myPhantomBlock.find("tbody").append($("<tr>").append($("<td>").append(phantomPlaybookText)) )
                    $("#related_content").append(myPhantomBlock)
                        ...
                        smResults.on("data", function (properties) {
                            ...
                            if (typeof data[0] != "undefined" && typeof data[0].count != "undefined" && data[0].count > 0) {
                                ...
                                var launchLink = $('<button style="float: right;" class="btn btn-primary">Open in ES <i class="icon-external" /></button>').click(function () {
                                    window.open(
                                        Splunk.util.make_full_url('/app/SplunkEnterpriseSecuritySuite/correlation_search_edit?search=' + data[0].title),
                                        '_blank'
                                    );
                                })
                ...
                if (ShowcaseInfo['summaries'][SummaryName].app == "Splunk_User_Behavior_Analytics") {
                    ...
                    for (let i = 0; i < storyID.length; i++) {
                        ...
                        tableData += '<div data-story-name="' + storyDetails["name"].replace(/"/g, "\\\"") + '" class="escu_button"></div>';
                        ...
                        if(storyDetails["support"].length != 0) {
                            ...
                            tableData += "<li>" + storyDetails["support"] + "</li>";
                            }
                            tableData += "</ol>";
                        }
